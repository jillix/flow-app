...
function _require( file ){
	...
	try { return require( file ) }
		...
		if( process.env.DEV ) console.log( "File: " + file + "\n" + err );
...
this.read = function( file, encoding, callback ) {
	...
	if( fileCache[ file ] ) callback( null, fileCache[ file ] );
	else fs.readFile( file, encoding, function( err, data ){
			...
			fs.watchFile( file, function( curr, prev ) {
			    ...
			    if( curr.mtime.valueOf() != prev.mtime.valueOf() || curr.ctime.valueOf() != prev.ctime.valueOf() ) {
			    	...
			    	fs.readFile( file, encoding, function( err, data ){
			    		...
			    		if( err ) fs.unwatchFile( file );
			    		else fileCache[ file ] = data;
			    	});
			    }
			});
