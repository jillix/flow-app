...
function middleware (connectionHandler) {
    ...
    return function (connection, response) {
        ...
        connectionHandler(null, session || clone(Session), connection, response);
    };
}
...
function uid (len) {
    ...
    for (; i < len; ++i) {
        string += '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'[0 | Math.random() * 62];
    }
    return string;
}
