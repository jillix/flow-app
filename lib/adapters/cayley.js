            ...
            g.V(module_iri).Out([
                ...
                module.forEach(item => {
                    if (item.predicate === vocab + 'gitRepository') {
                        ...
                        name = item.id.slice(1, -1);
                    }
                });

                if (installed[module_iri]) {
                    try {
                    ...
                    readdir(entrypoint.module_root, (err, items) => {

                        if (err) {
                            return cb(err);
                        }

                        if (items.find(item => {return item === name})) {
                            try {
                                installed[module_iri] = true;
                                cb(null, require(entrypoint.module_root + name));
                            } catch (e) {
                                cb(e);
                            }
                        ...
                        exec('npm i --prefix ' + entrypoint.base + ' ' + repo + '#flow_v0.1.0', err => {
