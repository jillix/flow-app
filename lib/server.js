...
var WebSocketServer = require('websocket-stream').Server;
...
module.exports = function (config) {
    ...
    var wss = new WebSocketServer({server: server}, function (stream) {

        // plug client session midleware
        stream.pause();
        clientSession(stream.socket.upgradeReq, {}, function (err) {

            // setup flow on socket
            stream.app = app;

            // multiplexer for flow event streams
            stream.pipe(CoreInst.demux(CoreInst, stream)).pipe(stream);
