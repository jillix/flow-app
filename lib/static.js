...
exports.client = function (stream) {
    ...
    File.prepare(loadContext, function (err, files) {
        ...
        if (err) {
            ...
            return clientResponse(stream, 500, err.toString());
        }
        ...
        gzip(
            ...
            function (err, data) {
                ...
                clientResponse(stream, 200, client);
            }
        );
    });
};

function clientResponse (stream, code, data) {
  
    // send to buffered requests
    if (reqBuffer.length) {
        for (var rb = 0; rb < reqBuffer.length; rb++) {
            reqBuffer[rb].end(code, data);
        }
        reqBuffer = [];
    } 
    
    // send client
    stream.end(code, data);
}
...
exports.composition = function (stream) {
    ...
    // receive data
    stream.data(function (instance) {

        // load entrypoint module instance with hostname
        if (!instance) {

            instance = stream.socket.upgradeReq.headers.host.split(':')[0];
        ...
        Composition(instance, related, role, function (err, composition) {
            ...
            if (!composition.client.module) {
                return stream.write(
                    engine.log('E', 'Static: Composition "' + composition.name + '" has no client module.')
                );
            }
...
exports.file = function (stream) {
    ...
    var file;
    if (!(file = createModuleFileDescriptor(stream))) {
        return stream.end(404, 'File not found. TODO: look in public folder for requested file.');
    }
