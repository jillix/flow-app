...
var markupCache = engine.cache.file('markup', {wd: engine.repo + 'markup/'});
...
function getHtml (link) {
    ...
    // get module composition
    var module_instance = moduleInstanceCache.get(link.path[0], link.role);

    // check if module exists
    if (!module_instance || !module_instance._client || !module_instance._client.html) {
        link.headers = {'content-type': 'text/plain'};
        return link.end(404, 'File not found.');
    }

    // save compressed/compiled file in cache and send it to the client
    markupCache.set(
        ...
        function (err, template) {
            ...
            link.headers = template.http;
            link.end(200, template.data);
        },

        // pass an array of allowed files
        module_instance._client.html
    );
    ...
    /*link.data(function (err, path) {
        ...
        markupCache.set(path, function (err, snippet) {

            if (err || !snippet && !snippet.data) {
                return link.send(err || 'HTML snippet not found.');
            }
