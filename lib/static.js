...
exports.public = function (pathname, req, res) {
    ...
    engine.file.get(pathname.substr(1), function (err, file) {
...
exports.moduleFile = function (link) {
    ...
    var filePath = link.path.slice(1).join('/');
    var file = {
        path: filePath,
        base: modulePackage._base,

        // wrap path must be the same as the module path ./static.js#221
        wrap: link.path[0] + '/' + filePath
    };
    ...
    /*if (options.allowedFiles) {
        ...
        for (var i = 0; i < options.allowedFiles.length; ++i) {
            if (options.allowedFiles[i].replace(/^\.\//, '') === (options.prependPath || '') + path) {
                found = true;
                break;
            }
        }
