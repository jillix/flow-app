 ...
 */
module.exports = function start (repo, port, logLevel, production) {
    ...
    engine.http.server.listen(port, function () {
        ...
        engine.mind(['E', Static.client])
              .mind(['P', Static.public])
              .mind(['F', Static.file])
              .mind(['W', Static.wrap])
              // TODO load html snippted over websocket,
              //      until html imports are supported
              .mind(['M', Static.markup])
              .mind(['C', Static.composition]);
