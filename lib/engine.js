...
var engine = global.engine = utils.clone(Flow);
engine._events = {};
...
module.exports = function start (repo, port, production) {
    ...
    engine.http.server.listen(port, function () {
        ...
        engine.on('client', {call: Static.client});
        engine.on('public', {call: Static.public});
        engine.on('file', {call: Static.moduleFile});
        engine.on('extFile', {call: Static.externalFile});
        
        // load html snippted over websocket until html imports are supported
        engine.on('html', {call: Static.markup});
        
        // core websocket module instance load interface
        engine.on('load', {call: Instance.load});
