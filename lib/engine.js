 ...
 */
module.exports = function (repo, port, user, key, production) {
    ...
    engine.http.server = http.createServer(engine.session(engine.http.request));

    // create a websocket server
    engine.socket.server = new WebSocketServer({server: engine.http.server});

    // listen to websocket connections
    engine.socket.server.on('connection', engine.session(engine.socket.connection));

    // start http server
    engine.server.http.listen(port);
