...
var Stream = {
    ...
    data: function (method, instance) {
        var args = [].slice.call(arguments);
        var scope = this._;
        
        args.splice(0, 1);
        if (instance && typeof instance._flowFn === 'object') {
            scope = instance;
            args.splice(0, 1);
        }
        
        this._d.push([scope, method, args]);
        return this;
    },
    ...
    emit: function (err, data, ignore) {
        ...
        for (i = 0, l = this._d.length; i < l; ++i) {
            ...
            data = this._d[i][1].apply(this._d[i][0], [err, data, this].concat(this._d[i][2])) || data;
        }
