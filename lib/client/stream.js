...
var Stream = {
    ...
    emit: function (err, data, ignore) {
        ...
        for (i = 0, l = handlers.length; i < l; ++i) {
            ...
            returnValue = handlers[i][1].apply(handlers[i][0], [argData, this].concat(handlers[i][2]));
...
//utils.nextTick(function () {
        ...
        if (self._i) {
            self._i.end.apply(self._i, arguments);
        }

        // TEMPORARY CODE
        // =================================================================
        if (self._o.length) {
            for (var i = 0; i < self._o.length; i++) {
                if (!self._o[i]._ended) {
                  self._o[i].end.apply(self._o[i], arguments);
                }
            }
        }
        engine._endStreamStat(self._streamKey);
