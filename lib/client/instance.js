 ...
 */
module.exports = function factory (module, config, role, callback, isDep) {
    ...
    var readyHandler = function (err) {
        ...
        if (--countDown === 0) {
            initInstancesInLoadOrder(instance, callback, isDep);
        }
    };
    ...
    if (config.load) {
        ...
        for (var i = 0, l = config.load.length; i < l; ++i) {
            engine.load(config.load[i], role, readyHandler, true);
        }
    }
 ...
 */
function initInstancesInLoadOrder (instance, callback, isDep) {

    // init instance
    !isDep && setupFlowAndInit(instance);
