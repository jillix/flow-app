...
module.exports = function factory (module, config, role, callback, loaderInstance) {
    ...
    instance.log = require('./logs') || dummyFn;
...
function loadSnippets (urls, callback) {
    ...
    for (var i in urls) {
        if (!markups[urls[i]]) {
            ...
            utils.nextTick(stream.end.bind(stream));
            callback();
        }
    }
}
