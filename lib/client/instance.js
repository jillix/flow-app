 ...
 */
module.exports = function factory (module, config, callback) {
    ...
    var readyHandler = function (err) {
        ...
        if (--countDown === 0) {
            initInstancesInLoadOrder(instance, callback);
        }
    };
    ...
    // append external flow config
    if (config.extFlow) {
        instance._extFlow = config.extFlow;
    }
    ...
    if (config.load) {
        ...
        for (var i = 0, l = config.load.length; i < l; ++i) {
            engine.load(config.load[i], readyHandler);
        }
    }
...
function initInstancesInLoadOrder (instance, callback) {
    ...
    // init dependencies
    var dependencies = instance._config.load;
    if (dependencies) {
