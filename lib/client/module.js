...
(function (global, body, state) {
    ...
    engine.module = function (name) {
        ...
        // save new empty module in cache, to prevent overlapping requests
        var module = engine.modules[name] = engine.clone(Module);

        // create a link
        var link = engine.link('load', function (err, data) {});
        ...
        // get module instance configuration
        engine.link('M>', name, function (err, config) {
            ...
            // setup links
            if (config.links) {
                for (var i = 0, link; i < config.links.length; ++i) {
                    link = config.links[i];
                    module.on(link.event, engine.link(link), link['1']);
                }
            }
        });
    };
