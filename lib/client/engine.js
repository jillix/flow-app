...
(function (global, body, state) {
    ...
    engine.load = function (scripts, module_instance, callback) {
        ...
        for (var i = length - 1, url, cleanPath, fingerprint; i >= 0; --i) {
            ...
            cleanPath.splice(cleanPath.length - 2, 1)[0];
        ...
        return function (name) {
            ...
            switch (name.indexOf('./')) {
                ...
                case 1:
                    ...
                    // create new path
                    name = basePath.slice(0, stepsBack - 1).join('/') + '/' + name.pop();
            }
    ...
    engine.html = function (urls, callback) {
        ...
        var loadedHandler = function (url, linkElement) {

            // save imported document
            engine.htmls[url] = linkElement.import.body.innerHTML;
        ...
        for (var i in urls) {
            if (!engine.htmls[urls[i]]) {
                engine.htmls[urls[i]] = 1;

                var url = (urls[i][0] !== '/' ? '/@/@/' : '') + urls[i];
                var link = body.createElement('link');
                link.setAttribute('rel', 'import');
                link.setAttribute('href', url);
                body.head.appendChild(link);

                url = engine.cleanPath(url);

                engine.on(url, loadedHandler, 1);

                link.onload = (function (url, link) {
                    return function () {
                        engine.emit(url, url, link);
                    }
                })(url, link)
            }
        }
    ...
    engine.css = function (urls) {
        ...
        for (var i = 0, link; i < urls.length; ++i) {
            if (!engine.csss[urls[i]]) {
                ...
                urls[i] = (urls[i][0] === '/' || urls[i].indexOf('://') > 0 ? '' : '/@/@/') + urls[i];

                link.setAttribute('rel', 'stylesheet');
                link.setAttribute('href', (urls[i][0] !== '/' ? '/@/@/' : '') + urls[i]);
                body.head.appendChild(link);
            }
        }
    };
