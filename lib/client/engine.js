...
(function (global, body, state) {
    ...
    engine.module = function (name, tempLoadingCache) {
        ...
        var link = engine.link('load', function (err, config) {
            ...
            if (config.html) {
                ...
                // load html snippets
                engine.html(config.name, config.html, function (err) {
    ...
    function htmlImportLoaded (event, data) {
     ...
     */
    engine.html = function (module_instance_name, urls, callback) {

        var count = urls.length;
        var loadedHandler = function (url, linkElement) {

            // save imported document
            engine.htmls[url] = linkElement.import;
        ...
        for (var i in urls) {
            if (!engine.htmls[urls[i]]) {
                ...
                var url = '/@/0/html/' + module_instance_name + urls[i];
                var link = body.createElement('link');
                link.setAttribute('rel', 'import');
                link.setAttribute('href', url);
                body.head.appendChild(link);

                engine.on(urls[i], loadedHandler, 1);
