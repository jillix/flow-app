...
(function (global, body, state) {
    ...
    engine.load = function (scripts, module_instance, callback) {
        ...
        // get the module name
        var moduleName = module_instance._config.module;
     ...
     */
    engine.module = function (name, parent) {
        var self = this;
        ...
        // save a reference to the parent instance
        module_instance._parent = parent || null;
        ...
        var link = engine.link('load', function (err, config) {
            ...
            module_instance._config = config;

            // load css
            config.css && engine.css(config.css);

            // load scripts
            config.scripts && engine.load(config.scripts, module_instance, loadHandler);

            // load sub modules
            if (config.load) {
                for (var i = 0; i < config.load.length; ++i) {
                    engine.module(config.load[i], module_instance);
                }
            }
     ...
     * @param {mixed} The following arguments are passed to the event handlers.
     */
    function loadHandler (err, module_instance) {

        // all scripts are loaded, module instance is now ready

        // check if all submodules are laoded and then setup the event flow
