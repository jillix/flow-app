...
(function (global, body, state) {
    ...
    function loadHandler (err, tempLoadingCache) {
        ...
        if (!tempLoadingCache.count) {
            tempLoadingCache.count = 0;
        }
        ...
        if (++tempLoadingCache.count === tempLoadingCache.length) {
            ...
            for (var i = 0; i < tempLoadingCache.length; ++i) {

                // setup event flow
                if (tempLoadingCache[i]._config.flow) {
                    engine.flow(tempLoadingCache[i], tempLoadingCache[i]._config.flow);
                }
