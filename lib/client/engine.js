...
(function (global, body, state) {
    ...
    var engine = global.E = function Engine (path, module) {
        ...
        // after all resources are loaded.
            ...
            engine.scripts[path] = module;
            engine.emit(path).send(null, path);
        }
    };
    ...
    function createEventStream (event, endHandler) {
        
        var es = engine.clone(EventStream);
        es.event = event;
        es._end = endHandler;
        es._h = [];
        return es;
     ...
     */
    function emit (event, endHandler, state) {
        ...
        var eventStream = createEventStream(event, endHandler);
    ...
    function on (event, handler, once, noRoute) {
        ...
        if (typeof handler === 'function') {
            ...
            if (!events[event]) {
                ...
                events[event].re = new RegExp(event.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g, "\\$&"));
    ...
    engine.route = function (url, event, data, fromPopstate) {
        ...
        engine.emit({event: url, all: true}, null, url).send(null, stateEvent);
    };
    ...
    engine.load = function (scripts, module_instance, callback) {
        ...
        var checkLoaded = engine.emit('loadScripts', function (err, data) {});
    ...
    function loadHandler (err, loadedModuleInstances) {
        ...
        if (++loadedModuleInstances.count === loadedModuleInstances.length) {
            ...
            for (var i = 0, l = loadedModuleInstances.length, inFlow, moduleInstance; i < l; ++i) {
                moduleInstance = loadedModuleInstances[i];
                inFlow = moduleInstance._flow;

                // setup event flow
                if (inFlow) {

                    for (var f = 0, fl = inFlow.length, config; f < fl; ++f) {
                        config = inFlow[f];
                ...
                if (outFlow) {
                    ...
                    for (var name in outFlow) {
                        moduleInstance.out[name] = engine.flow(module_instance, outFlow[name]);
                    }
                }
