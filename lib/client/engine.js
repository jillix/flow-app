...
(function (global, body, state) {
    ...
    engine.module = function (name, tempLoadingCache) {
        ...
        if (name) {
            engine.modules[name] = module_instance;
        }
        ...
        var link = engine.link('load', function (err, config) {
            ...
            if (err) {
                ...
                name && delete engine.modules[name];
            ...
            // save the flow config for external events, like DOM events
            ...
            if (!name) {
                engine.modules[config.name] = module_instance;
            }
            ...
            if (config.html) {
                ...
                // save the flow config for external events, like DOM events
                module_instance._html = {
                    flow: config.html.flow
                };
                ...
                // load html snippets
                engine.html(config.html.tmpl, function (err, html) {

                    // save html template on module instance
                    module_instance._html.tmpl = html;

                    // continue with loadind
     ...
     */
    engine.html = function (url, callback) {

        // check if url is already loaded
        if (engine.htmls[url]) {
            return callback(null, engine.htmls[url])
        }

        // create link
        var link = engine.link('html', function (err, html) {

            // errors
            if (err || !html) {
                return callback(err || new Error('No HTML template.'));
            }

            // save html template in cahce and return it
            callback(null, engine.htmls[url] = html);
        });
