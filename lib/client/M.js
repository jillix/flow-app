...
var M = (function() {
    ...
    var constructor = function (target, miid, dataContext, callback) {
        ...
        // WEBSOCKET TEST
        modules['testMiid'] = Object.extend({name: 'ws test module'}, Mono);
        modules['testMiid'].on('trucken', function (err, data) {
...
function link (operation, data) {
    try {
        webSocket.send(JSON.stringify(['testMiid', operation, data]));
    } catch (err) {
