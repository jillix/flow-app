...
(function(instances, modules, models, views, wsCache, css, fn, win, doc) {
    ...
    webSocket.onmessage = function (message) {
        ...
        if (instance && event && instances[instance]) {
            ...
            // emit event
            instances[instance].Z.emit(event, err, data);
        }
    };

    // -------------------------------------------- SEND TO SERVER EVENT HANDLER
    ...
    // load and create element instances
    function load (type, name, callback) {
        var self = this;
        var factory;
        var clone;
        var cacheKey;
        ...
        switch (type) {
            ...
            case 'view':
                ...
                // get view from cache
                if (views[cacheKey]) {
                    return views[cacheKey];
                }

                clone = views[cacheKey] = self.clone(View);
                factory = viewFactory;
                break;
            ...
            case 'inst':
                ...
                clone = instances[cacheKey] = self.clone(Instance);
            ...
            case 'model':
                ...
                clone = models[cacheKey] = self.clone(Model);
                factory = modelFactory;
                break;
        }

        // get factor config from server
        self.emit(type + '>', null, name, function (err, config) {
            ...
            // start observing
            if (config.observe) {
                observe.call(self, clone, config.observe);
            }

            // ?
            if (config.act) {
                react.call(self, clone, config.act);
            }

            // factory clone
            factory.call(clone, config);
        });
    }

    // set up observables
    function observe (clone, config) {
        var self = this;
    }

    // react
    function react (clone, config) {
        var self = this;
    }

    // instance
    function instanceFactory (clone, config) {
        // add send handler to "inst>" event
    }
        ...
        self.emit('inst>', null, name, function (err, config) {
            ...
            if (config.scripts && config.scripts.length > 0) {
                loadJS(config.module, config.scripts, function (moduleConstructor) {
                    ...
                    initInstance(instance, moduleConstructor, function () {
...
*/
        ...
        Z: {
            ...
            route: function (url, fromPopstate) {
                ...
                for (var instance in instances) {
                    ...
                    if (!instances[instance].Z.ready || (instances[instance].Z.name !== stateEvent.ori && instances[instance].Z.url === url)) {
                        continue;
                    }

                    // set current url
                    instances[instance].Z.url = url;

                    // emit url route event
                    instances[instance].Z.emit.call(instances[instance], url, stateEvent);

                    // emit general route event
                    instances[instance].Z.emit.call(instances[instance], 'route', stateEvent);
                }
            },
                ...
                for (var _event in events) {
                    ...
                    if (_event === event || events[_event].re.test(event)) {
                        ...
                        if (events[_event]._mods) {
                            for (i = 0; i < events[_event]._mods.length; ++i) {
                                self.load(events[_event]._mods[i]);
                            }
                            events[_event]._mods = null;
                        }

                        for (i = 0; i < events[_event].length; ++i) {
                            if (events[_event][i]) {
                        ...
                        // routes on the same instance are (most of the time) unique
                        if (route) {
                            break;
                        }
                    }
            ...
            push: function (instance, event) {
                if (instances[instance]) {
                    instances[instance].Z.emit.apply(instances[instance], this.toArray(arguments).slice(1));
                }
            },
            ...
            spill: function (event) {
                ...
                for (var instance in instances) {
                    instances[instance].Z.emit.apply(instances[instance], args);
                }
            },
    ...
    function createViewInstance (cacheKey, config, callback) {
        ...
        if (config.model) {
            ...
            self.Z.emit('model>', 0, config.model, function (err, schema) {
                ...
                models[modelCache] = self.Z.flat(schema);
    ...
    // add listeneres
    function listen (event, handler, options, staticArgs, view, modules, once) {
        var self = this;
    ...
    // load scripts
    function initInstance (instance, init, callback) {
        ...
        if (typeof init === fn) {
            ...
            if (instance.Z.config.modules) {
                paralellReadyHandler(instance.Z.config.modules, function () {
            ...
            // handle waitFor
            paralellReadyHandler(instance.Z.config.waitFor, function () {
                init.call(instance);
            });
    ...
    // save core module on instances
    win.Z = instances.Z = coreInstance;
