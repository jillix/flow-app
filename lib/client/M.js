        ...
        if (mods.length) {
            ...
            var modLoaded = function () {
                ...
                if(--length === 0) {
                    ...
                    // eveluate scripts in order
                    for (var i = 0, l = mods.length; i < l; ++i) {
                        moduleScripts[miid + i] = moduleScripts[miid + i](require, {});
                    }
                    
                    callback(moduleScripts[miid + 0]);
                }
            };
            ...
            for (var i = 0, l = mods.length; i < l; ++i) {
                ...
                node.charset = "utf-8";
                node.async = true;
                node.src = "/" + Mono.ok + "/core/getFile/" + mods[i];
        ...
        Mono.link("getConfig", { miid: "core", path: miid }, function (err, config) {
            ...
            if (modulesToLoad.length > 0) {
                ...
                loadJS(miid, modulesToLoad, function (moduleConstructor) {
                    ...
                    // create module
                    if (config.path) {
                        modules[miid].__init__ = hasModuleMainScript ? moduleConstructor : null;
