...
(function(modules, cache, wsCache, css, fn, win, doc) {
    ...
    function load (type, name, callback) {
        ...
        if (typeof name === 'object') {
            return factoryHandler.call(self, factory, self.clone(klass[type]), name, callback);
        }
    ...
    function factoryHandler (factory, clone, config, callback) {
        ...
        if (config.L) {
            ...
            for (var i = 0; i < config.L.length; ++i) {
                load.call(clone, observe.config, handler);
            }
        }
    ...
    function act (config) {
        ...
        return function (callback) {
            ...
            // load (before init)
            if (config.load) {
                load.call(self, config.load.type, config.load.name, callback);
            }
        };
    }
