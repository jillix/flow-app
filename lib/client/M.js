...
(function(instances, modules, models, views, wsCache, css, fn, win, doc) {
    ...
    function load (type, name, callback) {
        ...
        switch (type) {
            ...
            default:
                return callback('load: invalid type');
        }
        ...
        self.emit(type + '>', null, name, function (err, config) {
            ...
            // start observing
            if (config.O) {
                observe.call(self, clone, config.O);
            }

            // act
            if (config.A) {
                act.call(self, clone, config.A)();
            }
        });
    }
    ...
    function observe (clone, config) {
        ...
        // actions (dom)
        if (config.events) {
            // TODO get selectors after dom is rendered
        }
    }

    // act (handlers)
    function act (clone, config) {
        var self = this;
        var method;
...
*/
    ...
    function paralellReadyHandler (mods, callback) {
        ...
        if (mods instanceof Array) {
            ...
            for (var i = 0; i < count; ++i) {
                instances[mods[i]].on('ready', handler, 1);
            }
        } else {
            callback();
        }
    ...
    function loadJS (moduleName, moduleSources, callback) {
        ...
        for (var i = length - 1, source, url; i >= 0; --i) {
            ...
            instances.Z.on(source, modDepLoaded, 1);
