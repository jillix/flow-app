...
(function(modules, cache, wsCache, css, fn, win, doc) {
        ...
        I: function (inst, config, callback) {
            ...
            if (config.modules) {
                for (var m = 0; m < config.modules.length; ++m) {
                    inst.load(config.modules[m], name);
                }
            }
            ...
            if (config.scripts && config.scripts.length > 0) {
                loadJS(config.module, config.scripts, function (moduleConstructor) {
                    ...
                    initInstance(inst, moduleConstructor, function () {
    ...
    // create core module
    var coreInstance = klass.I.Z.clone(klass.I);
    ...
    function load (type, name, callback) {
        ...
        if (typeof name === 'object') {
            return factoryHandler.call(self, self.clone(klass[type]), name, callback);
        }
        ...
        // create clone
        clone = cache[type][cacheKey] = self.clone(klass[type]);
        ...
        self.emit(type + '>', null, name, function (err, config) {
            ...
            // call factory
            factoryHandler.call(self, clone, config, callback);
        });
    }

    // factory handler
    function factoryHandler (clone, config, callback) {
        var self = this;

        // factory clone
        factory.call(self, clone, config, function (err) {
