...
var M = (function() {
        ...
        */
        link: function(method, options, callback) {
            ...
            if (method.indexOf('/') === 0) {
                ...
                url = "/" + ok + "/" + (options.miid || this.miid) + "/" + method + (options.path || options.path === "" ? "/" + options.path : "") + (options.query || "");
            }
    ...
    function require (miid, module) {
        ...
        return function (name) {
            ...
            if (name.indexOf('..') === 0) {
                ...
                // check, that requested module lies within the root dir
                if (stepBackLenght > module.path.length) {
                    return;
                }
                
                name = module.base + (stepBackLenght === module.path.length ? namePath : module.path.slice(0, stepBackLenght).join('/') + '/') + namePath;
            }
    ...
    function evaluateScriptsInOrder (miid, moduleSources, callback) {
        ...
        callback(moduleDeps[miid][moduleSources[0]].exports);
    }
    ...
    var constructor = function (target, miid, callback) {
        ...
        Mono.link("getConfig", { miid: "core", path: miid }, function (err, config) {
            ...
            if (config.scripts && config.scripts.length > 0) {
                ...
                //config.scripts.push('/test1.js');
                //config.scripts.push('/testA/test2.js');
                //config.scripts.push('/testA/testB/test3.js');
                config.scripts.push('http://ajax.googleapis.com/ajax/libs/jquery/2.0.0/jquery.min.js');
