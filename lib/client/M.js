...
var M = (function() {
    ...
    var Mono = {
        ...
        */
        ...
        // emit event
        /*
            this.emit("setSomething", [miid], arg1, arg2, ...);
        */
        emit: function(event, miid) {
            
            // slice first argument and apply the others to the callback function
            var args = Array.prototype.slice.call(arguments).slice(miid ? 2 : 1);
            ...
            if (events[miid] && events[miid][event]) {
                ...
                for (var i = 0, l = events[miid][event].length; i < l; ++i) {
                    ...
                    if (events[miid][event][i]) {
                        ...
                        events[miid][event][i].apply(miid ? module[miid] : this, args);
                    }
                }
    ...
    function initModule(miid, config) {
        
        // call module constructor
        if (typeof moduleConstructors[miid] === "function") {
            moduleConstructors[miid].call(modules[miid], config);
        }
        ...
        Mono.link("getConfig", { miid: "core", path: miid }, function (err, config) {
            ...
            if (config.html) {
                ...
                htmlReq.onload = function (html) {
                    ...
                    if (moduleConstructors[miid]) {
                        initModule(miid, config);
                    }
                };
                htmlReq.send();
            }
            ...
            if (!moduleConstructors[miid] && config.path) {
            ...
            if (config.scripts && config.scripts.length) {
                ...
                for (var i = 0, l = config.scripts; i < l; ++i) {
            ...
            if (modulesToLoad.length > 0) {
                ...
                require(modulesToLoad, function (moduleConstructor) {
                    ...
                    if (config.path) {
                        ...
                        moduleConstructors[miid] = moduleConstructor;
