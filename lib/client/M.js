...
/*{
    ...
    methods..
        ...
        I: {
            ...
            _load: function (type, name, callback) {
                ...
                if (typeof name === 'object') {
                    return factoryHandler.call(self, factory, self._clone(classes[type]), name, callback);
                }
            ...
            route: function (url, fromPopstate) {
                ...
                for (var instance in cache.I) {
                    ...
                    if (!cache.I[instance].ready || (cache.I[instance]._name !== stateEvent.ori && cache.I[instance].url === url)) {
                        continue;
                    }

                    // set current url
                    cache.I[instance].url = url;
    ...
    // setup defailt as server events
    ...
    function factoryHandler (factory, clone, config, callback) {
        ...
        if (config.L) {
            ...
            for (var i = 0; i < config.L.length; ++i) {
                self._load(config.L[i].type, config.L[i].name, handler);
            }
        }

        // load scripts and init module
        if (config.scripts && config.scripts.length > 0) {
            return loadJS(config.module, config.scripts, handler);
        }
        ...
        return function (callback) {
            ...
            if (config.L) {
                for (i = 0; config.L.length; ++i) {
                    self._load(config.L[i].type, config.L.name);
                }
            }
        };
    }
