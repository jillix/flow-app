(function(modules, cache, wsCache, css, fn, win, doc) {
        ...
        I: {
            ...
            _load: function (type, name, callback) {
                ...
                if (typeof name === 'object') {
                    ...
                    return loader.call(self, factory, self._clone(classes[type]), name, callback);
                }
                ...
                self.emit(sendEvent + '>', null, name, function (err, config) {
                    ...
                    // call factory
                    loader.call(self, factory, clone, config, callback);
                });
            },
    ...
    function loader (factory, clone, config, callback) {
        var self = this;
        var ready = 1;
        var count = 0;
        var init;
        var handler = function (err, constructor) {
            ...
            if (err) {
                return callback(err, constructor);
            }
    ...
    function act (config) {
        ...
        return function () {
            ...
            // load
            if (config.L) {
                for (i = 0; i < config.L.length; ++i) {
                    self._load(config.L[i].type, config.L[i].name);
                }
            }
        };
    }
