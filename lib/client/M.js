...
var M = (function() {
    ...
    function loadJS (miid, mods, callback) {
        console.log(mods);
        if (mods.length) {
            ...
            var require = function (name) {
                ...
                for (var script in moduleScripts[miid]) {
                    
                    if (script.indexOf(name) > -1) {
                        return moduleScripts[miid][script];
                    }
                }
            };
            ...
            var modLoaded = function () {
                ...
                if(--length === 0) {
                    ...
                    for (var i = (mods.length - 1), l = 0; i >= l; --i) {
                        
                        if (typeof moduleScripts[miid][mods[i]] === 'function') {
                            var module = {exports: {}};
                            moduleScripts[miid][mods[i]] = moduleScripts[miid][mods[i]](require, module, module.exports);
                        }
                    }
                    
                    callback(moduleScripts[miid][mods[0]]);
                }
            };
    ...
    var constructor = function (target, miid, callback) {
        ...
        Mono.link("getConfig", { miid: "core", path: miid }, function (err, config) {
            ...
            if (!modules[miid].__init__ && config.path) {
                modulesToLoad.push('/' + ok + '/core/getModule/' + config.path + (config.file ? '/' + config.file : ''));
            }
            
            // load scripts
            if (config.scripts && config.scripts.length) {
                 modulesToLoad = modulesToLoad.concat(config.scripts[i]);
            }
            
            // TEMPORARY TESTING OF MONO MODULE LOADER
            //modulesToLoad = ['test1.js', 'test2.js', 'test3.js'];
    ...
    constructor.wrap = function (miid, name, module) {
        
        if (!moduleScripts[miid]) {
            moduleScripts[miid] = {};
        }
        
        moduleScripts[miid][name] = module;
    };
