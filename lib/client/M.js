...
(function(instances, modules, wsCache, fn, win, doc) {
    ...
    var Mono = {
        ...
        route: function (event, fromPopstate) {
            ...
            /* TODO is this still needed ? (from the old state module)
            // ignore states that have already this state, except on the states with the same instance
            if (state._cu === url && instance !== state._mi.mono.name) {
                continue;
            }
            */
            
            // emit url event on all instances
            self.spill(url, stateEvent);
            
            // emit route event
            self.spill('route', stateEvent);
        },
