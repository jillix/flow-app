...
(function(instances, modules, models, views, wsCache, css, fn, win, doc) {
    ...
    var Module = {
            ...
            for (var _event in events) {
                ...
                if (_event === event || events[_event].re.test(event)) {
                    ...
                    if (args[0] && args[0].url && events[_event]._map) {
                        // get map
                        args[0].map = self.mapUrl(events[_event].re, events[_event]._map);
                    }
        ...
        model: function (name, project, callback) {
            ...
            self.emit('model>', 0, name, function (err, schema) {
        ...
        view: function (name, project, callback) { // TODO fix project argument to createViewInstance (project is now the callback in some cases)
            var self = this;
            ...
            if (typeof name === fn || typeof name === 'object') {
                callback = !project ? name : callback || project;
                return createViewInstance.call(self, null, name, project, !project ? name : callback || project);
            }
            ...
            self.emit('view>', 0, name, function (err, config) {
                ...
                createViewInstance.call(self, cacheKey, config, project, callback);
            });
        },
    ...
    function createViewInstance (cacheKey, config, project, callback) {
        var self = this;
        ...
        // load model
        if (config.model) {
            return self.model(config.model, project, function (err, model) {
    ...
    // add listeneres   
    function listen (event, handler, staticArgs, map, modules, once) {
        var self = this;
