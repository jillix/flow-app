...
(function(instances, modules, models, views, wsCache, css, fn, win, doc) {
    ...
    var Module = {
        ...
        emit: function(event) {
            ...
            for (var _event in events) {
                ...
                if (_event === event || events[_event].re.test(event)) {
                    ...
                    if (args[0] && args[0].pth && events[_event]._view) {
                        // get map
        ...
        model: function (name, project, callback) {
            ...
            if (arguments.length === 2) {
                ...
                if (typeof project === 'number') {
                    project = win.location.pathname.split('/')[project];
                }
        ...
        view: function (name, callback) {
            ...
            // check view cache
            if (views[name]) {
                return callback(null, views[name]);
            }
            ...
            self.emit('view>', 0, name, function (err, config) {
                ...
                createViewInstance.call(self, name, config, callback);
            });
        },
    ...
    function createViewInstance (cacheKey, config, callback) {
        ...
        // load model
        if (config.model) {
            return self.model(config.model, config.project, function (err, model) {
