...
var M = (function(miids, modules, wsCache, fn, win) {
    ...
    var Mono = {
        ...
        emit: function(event) {
            ...
            if (events && events[event]) {
                ...
                // remove listeners that only must be called once
                if (remove.length) {
                    for (i = 0; i < remove.length; ++i) {
                        if (events[remove[i][0]]) {
                            events[remove[i][0]].splice(remove[i][0], 1);
                        }
                    }
                }
            }
        ...
        push: function (miid, event) {
            if (miids[miid]) {
                miids[miid].emit.apply(miids[miid], convertToArray(arguments).slice(1));
            }
        },
        // emit on all miids
        ...
        miids.M.emit('load', null, miid, function (err, config) {
            ...
            if (config.scripts && config.scripts.length > 0) {
                loadJS(miid, config.name, config.scripts, function (miid, moduleConstructor) {
                    ...
                    initModule(miid, moduleConstructor);
                });
        ...
        if (typeof init === fn) {
            ...
            if (miid.config.waitFor instanceof Array) {
                ...
                var handler = function () {
                    if (++current === count) {
                        init.call(miids[miid.miid]);
                    }
                };
                
                for (var i = 0; i < count; ++i) {
                    miids[miid.config.waitFor[i]].one('ready', handler);
                }
            } else {
                init.call(miids[miid.miid]);
            }
    ...
    function send (event) {
        return function (err, data, callback) {
            ...
            var message = [this.mono.miid + ':' + event, err || 0];
            var cbId;
    ...
    miids.M = clone(Mono);
    miids.M.mono = {miid: 'M', ready: true};
