...
var M = (function() {
    ...
    function initModule (target, miid, config) {
        ...
        if (typeof modules[miid].__I === fn) {
            modules[miid].__I(config);
        }
    }
    ...
    function loadJS (miid, moduleSources, callback) {
        ...
        var modLoaded = function (src) {
            ...
            return function () {
                ...
                Mono.emit(src, moduleScripts, moduleSources, callback, --length);
            };
        };
        ...
        for (var i = moduleSources.length - 1; i >= 0; --i) {
            ...
            Mono.once(moduleSources[i], (function (miid) {
                
                return function (moduleScripts, moduleSources, callback, length) {
            ...
            if (!moduleScripts[moduleSources[i]]) {
                ...
                if (node.attachEvent && !(node.attachEvent.toString && node.attachEvent.toString().indexOf('[native code') < 0)) {
                    node.attachEvent('onreadystatechange', modLoaded(moduleSources[i]));
                } else {
                    node.onload = modLoaded(moduleSources[i]);
                }
                
                /*
                if (typeof node.onload === 'undefined') {
                    node.onload = modLoaded(moduleSources[i]);
                } else {
                    node.onreadystatechange = (function (src) {
                ...
                if (typeof moduleScripts[moduleSources[i]] === fn || moduleScripts[moduleSources[i]] === 3) {
                    Mono.emit(moduleSources[i], moduleScripts, moduleSources, callback, length);
                }
            }
        }
    }
    ...
    var constructor = function (target, miid, callback) {
        ...
        Mono.link('getConfig', {miid: 'core', path: miid}, function (err, config) {
            ...
            modules[miid].__R = 0;
            modules[miid].__C = config;
            modules[miid].__T = target;
            ...
            if (config.html) {
                ...
                Mono.link(config.html, function (err, html) {
                    ...
                    if (++modules[miid].__R === 2) {
                        initModule(modules[miid].__T, miid, modules[miid].__C);
                    }
                });
            ...
            if (config.scripts && config.scripts.length > 0) {
                ...
                loadJS(miid, config.scripts, function (miid, moduleConstructor) {
                    ...
                    // create module
                    if (modules[miid].__C.path) {
                        modules[miid].__I = modules[miid].__I || moduleConstructor;
                        modules[miid].miid = miid;
                        modules[miid].path = modules[miid].__C.path;
