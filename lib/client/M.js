...
(function(instances, modules, wsCache, fn, win, doc) {
    ...
    window.addEventListener('popstate', function () {
        ...
        // emit route
        instances.M.route(win.location.pathname, true);
    }, false);
    ...
    var Mono = {
        // listen to events
        on: listen,
        one: function (event, handler, args) {
            listen.call(this, event, handler, args, true);
        },
        // remove listeners
        ...
        // route to url
        route: function (event, fromPopstate) {
            ...
            // emit url event only when url changes
            if (url === pathname) {
                return;
            }
        
            // set url to pathname when url points to the root
            if (!event) {
            ...
            // emit url event on all instances
            self.split(url, stateEvent);
        },
    ...
    // add listeneres
    function listen (event, handler, staticArgs, once) {
        var self = this;
        var events = this.mono.events;
        ...
        if (typeof handler === fn) {
            ...
            if (event === 'ready' && self.mono.ready) {
    ...
    // load scripts
    function initModule (instance, init, waitFor, subModules) {
    ...
    function load (instance, parentInstance) {
        ...
        instances.M.emit('load', null, instance, function (err, config) {
            ...
            if (err) {
                console.error(err);
                return delete instances[instance];
            }
            ...
            instances[instance].mono.config = config;
            instances[instance].mono.module = config.module;
            ...
            if (config.scripts && config.scripts.length > 0) {
                loadJS(config.module, config.scripts, function (moduleConstructor) {
                    ...
                    if (config.events) {
                        for (i = 0, l = config.events.length; i < l; ++i) {
                    ...
                    if (config.routes) {
                        for (i = 0, l = config.routes.length; i < l; ++i) {
                            instances[instance].on(config.routes[i].pattern, config.routes[i].handler, config.routes[i].args);
                        }
                    }
