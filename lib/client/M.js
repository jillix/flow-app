...
var M = (function() {
    ...
    function init (target, miid, config, dataContext) {
    ...
    function initModule (target, miid, config, dataContext) {
        ...
        if (config.waitFor instanceof Array) {
            ...
            for (var i = 0, l = config.waitFor.length; i < l; ++i) {
                ...
                currentMiid = config.waitFor[i];

                if (!modules[currentMiid] || modules[currentMiid].ready) {
                    if (++loaded === l) {
                        init(target, miid, config, dataContext);
                    }
                } else {
                    modules[miid].on('ready', currentMiid, function () {
                        if (++loaded === l) {
                            init(target, miid, config, dataContext);
                        }
                    }, true);
