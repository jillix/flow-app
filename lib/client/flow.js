 ...
 * @param {object} DOM data (optional).
 */
engine.flow = function (module_instance, outConfig, DOM) {
    ...
    return function handler (event, _data) {
        ...
        for (var i = 0, config, to; i < outConfig.length; ++i) {
            ...
            data = handleDataMerging(module_instance, config, data);

            // emit route
            if (config.route) {
                engine.route(parsePath(config.route, data), data);
            }
 ...
 */
function handleDomScope (event, DOM) {
    ...
    event._item = event._item || DOM.dataItem;
}
 ...
 */
function handleDataMerging (module_instance, config, data) {
    ...
    if (config.set) {
        ...
        for (var key in config.set) {
            ...
            // parse path
            var path = parsePath(config.set[key], event);
            ...
            // get search value with a path: 1. events, 2. data, 3. instance, 4. window
                ...
                data[key] = engine.path(path, event, true) || engine.path(path, data, true) || engine.path(path, module_instance);
            }
        }
...
function parsePath (path, event) {
    ...
    if (match) {
        for (var i = 0, value; i < match.length; ++i) {
            ...
            value = engine.path(match[i].replace(find_braces, ''), event);
