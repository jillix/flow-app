...
function Flow (originStream, context, config) {
    ...
    var stream = Stream(module_instance, context);
    ...
    if (typeof config.emit === 'string') {
        module_instance.flow(config.emit, null, stream);
    }
    ...
    if (config.call) {
        ...
        if (typeof config.call === 'string' && config.call.indexOf('/') > -1) {
            Socket.setup(stream, config.call);
        
        // call local method
        } else if ((config.call = getMethodFromPath(config.call, module_instance))) {
            config.call.call(module_instance, stream, config);
        }
    }
}
