...
module.exports = {
     ...
     */
    flow: function (eventName, readable, writable, context) {
        var events = this._events;
        
        context = context || {};
        
        // create streams
        var stream = Stream.Duplex(this, readable, writable);
 ...
 */
function Flow (originStream, config, context, adapter) {
    ...
    if (typeof config.to === 'string') {
        ...
        if (
            (!(module_instance = engine.instances[config.to])) ||
            (
            ...
            return console.error('Flow target instance "' + config.to + '" is not found.');
        }
    }
    
    // create a duplex stream for every event
    var stream = Stream.Duplex(module_instance);
    ...
    if (
        ...
        (config.data[0] = getMethodFromPath(config.data[0], module_instance))
    ) {
    ...
    if (typeof config.emit === 'string') {
        module_instance.flow(config.emit, stream, context);
    }
    ...
    if (config.call) {
        ...
        if (typeof config.call === 'string' && config.call.indexOf('/') > -1) {
            Socket.send.call(module_instance, config.call);
        
        // call local method
        } else if ((config.call = getMethodFromPath(config.call, module_instance))) {
            config.call.call(module_instance, stream, context, config);
        }
    }
}
