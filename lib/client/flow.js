...
function Flow (instance, stream, config) {
    ...
    for (var i = 1, l = config.length, flow, method, args, node; i < l; ++i) {
        flow = config[i];
        args = [];
        ...
        if (typeof method === 'function') {
            stream = callStreamHandler(stream, instance, method, args);
            continue;
        }
        ...
        if (typeof method[0] === 'string') {
            ...
            // load instance
            engine.load(method[0], role, function (err, instance) {
        ...
        if (session && !utils.roleAccess(instance, role)) {
            stream.write(engine.log('E', new Error('Flow target instance "' + instance._name + '" is not found.')));
            continue;
        }
        ...
        if (method[2]) {
            ...
            args.unshift(method[1]);
            stream.data.apply(stream, args);

        // call stream handler
        } else {
            stream = callStreamHandler(stream, method[0], method[1], args);
        }
    }
}

function callStreamHandler (stream, instance, method, args) {
    
    // add stream as first argument for stream handlers
    args.unshift(stream);
    
    //return method.apply(instance, args);
    if ((node = method.apply(instance, args)))  {
        
        // replace and connect stream with a returned stream from the stream handler
        stream = Stream(node._, node);
    }
