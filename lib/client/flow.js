...
function flowHandler (stream, event) {
    return this.flow(event, {stream: stream});
}
...
function Flow (instance, stream, config) {
    ...
    for (var i = 0, l = config.length, flow, method, args, node; i < l; ++i) {
        flow = config[i];
        args = [];
