...
function flowHandler (stream, event) {
    ...
    return stream._.flow(event, stream);
}
...
function Flow (instance, stream, config, flowStream, count) {
    ...
    if (typeof method[0] === 'string') {
        ...
        return engine.load(method[0], role, function (err, method_instance) {
            ...
            Flow(instance, callStreamHandler(stream, method, session, role), config, flowStream, count);
        });
    }
...
function callStreamHandler (stream, method, session, role) {
    ...
    if (method[3] === 1) {
        ...
        stream.data.apply(stream, [[method[0], method[1]]].concat(method[2]));
    ...
    // call stream handler
        ...
        // TODO check if more handlers follow
        if (typeof (node = method[1].apply(method[0], method[2])) === 'object')  {

            // replace and connect stream with a returned stream from the stream handler
            stream = Stream(node._, node, method[3] === 2 ? true : false).pause();
            node.resume();
        }
    }
...
function getMethodFromPath (module_instance, path) {
    ...
    if (
        typeof path === 'string' &&
        typeof (path = utils.path(path, [module_instance, module_instance._flowFn, global])) !== 'function'
    ) {
