...
function Flow (instance, originStream, config, context) {
    ...
    // load instance on flow event
    if (config.load) {
        for (var i = 0, l = config.load.length; i < l; ++i) {
            engine.load(config.load[i], role);
        }
    }
...
function createAndSetupStream (instance, originStream, config, context) {
    ...
    // return if flow has nothing to do
    if (!config.data && !config.call && !config.emit) {
        return;
    }
    ...
    // emit stream to other flow handlers
    /*if (typeof config.emit === 'string') {
        originStream.flow(config.emit, context, stream);
        //instance.flow(config.emit, context, stream);
    }*/
}
