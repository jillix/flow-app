 ...
 */
engine.flow = function (module_instance, flows, staticData, adapter) {
    
    /*
        TODO: - one data static/dynamic config per incoming event
              - move load and route out of the flow, one level up
        {
            "on": "pattern",
            "data": {},
            "set": {},
        ...
        IDEA: - make events strict again (events[event]) -> performance gain
              - handle routes separately (on(route, engine.flow())) -> better maintainabillity
    ...
    for (var option in flow) {
        ...
        if (option === 'data' || option === 'set') {
            ...
            */
            ...
            // parse and clone data/set config
            for (var key in flow[option]) {
                parsedFlow.data = {};

                // parse the data field values
                parsedFlow.data[key] = parsePath(flow[option][key], data, module_instance);

                // search for path in data and module instance and set it to the cloned flow
                if (option === 'set') {
                    parsedFlow.data[key] = getPathValue(parsedFlow[option][key], data, module_instance);
                }
            }
