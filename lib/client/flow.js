 ...
 */
module.exports = function (module_instance, config, adapter) {
    ...
    return function handler (eventStream) {
        ...
        if (config.flow && config.flow.length) {
            ...
            for (var i = 0, l = config.flow.length, flow, key; i < l; ++i) {
                ...
                // ..through streams? call could act as through handler
                // need a "to" for each pipe and call, though
                if (flow.pipe) {
                    eventStream.pipe(module_instance.link(flow.pipe, flow.to));
                }
                ...
                if (flow.call) {
                    ...
                    // get call function
                    if (typeof flow.call === 'string' && typeof (flow.call = getPathValue(flow.call, module_instance)) !== 'function') {
                        continue;
                    }
