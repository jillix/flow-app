...
var FlowEmitter = {
    ...
    flow: function (eventName, context) {
        ...
        if (typeof eventName === 'object') {
            utils.nextTick(Flow, this, createStream(this, context, stream), eventName, 0);
            return stream;
        }
 ...
 */
function Flow (instance, stream, config, count, originalStream) {
    ...
    if (typeof method[0] === 'string') {
        ...
        return engine.load(method[0], role, function (err, method_instance) {
            ...
            if (err) {
                return Flow(instance, stream, config, count, originalStream);
            }
            
            method[0] = method_instance;
            
            Flow(instance, callStreamHandler(stream, method), config, count, originalStream);
        });
    }
...
function callStreamHandler (stream, method) {
