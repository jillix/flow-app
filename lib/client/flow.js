 ...
 */
engine.flow = function (_module_instance, outFlow, adapter) {
    ...
    return function handler (eventStream) {
        ...
        for (var i = 0, l = outFlow.length, flow; i < l; ++i) {
            flow = outFlow[i];
            ...
            TODO: - parse config values {route}, {method}, etc..
            ...
            */
            ...
            if (flow.pipe) {
                eventStream.data(function (err, data) {
                    ...
                    // create event stream
                    if (typeof flow.pipe === 'string') {
                        flow.pipe = module_instance.emit(flow.pipe);
                    }
                    
                    // send data
                    flow.pipe.send(err, data);
                });
            }
