...
function setupStream (stream, url) {
    ...
    if (!url || typeof url === 'string' && !(url = parseUrl(url))) {
    ...
    stream.socket = stream.socket || getSocket(stream);
...
function getSocket (stream) {
    ...
    if (sockets[host]) {
        sockets[host].streams[stream.id] = stream;
        return sockets[host];
    }
 ...
 */
function send (err, data, stream, end) {

    // create message
    var message = [end ? 0 : 1, stream.url.inst, stream.url.emit, stream.id, err ? err.toString() : 0];
    ...
    // create socket a socket
    if (!stream.socket && stream.url) {
        stream.socket = getSocket(stream);
    }
    
    // buffer writes if socket is not ready yet
    if (stream.socket.readyState !== 1) {
        return stream.socket._buffer.push(message);
    }
...
function end (err, data) {
    ...
    // send end signal
    send.call(this._, err, data, this, true);
    ...
    */
    ...
    if (type) {
        ...
        if (!stream) {
            ...
            // create a new stream and save it in cache and write data
            if (instance._name === 'custom') {
                console.log('WS MSG NO STREAM:', event, id, instance._access || '@');
                debugger;
            }
...
function createStream (instance, event, id, socket) {
    ...
    var stream = instance.flow(event, {
        id: id,
        socket: socket
    });

    // create stream and save it to cache
    sockets[id] = setupStream(stream, {
        inst: instance._name || '@',
        emit: event
    });
...
function closeHandler (closeEvent) {
    ...
    for (var stream in socket.streams) {
        ...
        // remove socket ref on the stream
            ...
            socket.streams[stream].socket = undefined;
        }
    }
