(function(modules, cache, wsCache, css, fn, win, doc, win_location, cur_location, prev_location) {
    ...
    function loader (factory, clone, config, callback, sub, viewIndex) {
        ...
        if (config.load) {
            ...
            for (var i = 0, type; i < config.load.length; ++i) {
                ...
                // call loader
                self._load(type, config.load[i][1], loaderHandler, true, self._vi);
            }
        }
    ...
    function flow (config, onlyDom, onlyObs) {
        ...
        for (i = 0; i < config.length; ++i) {
            ...
            if (flow.selector && !onlyObs) {
                ...
                elm = doc.querySelectorAll(flow.selector);
                if (elm) {
    ...
    // act (handlers)
    function createHandler (flowConfig, dom, elmIndex, elms, dontPrevent) {
        var _self = this;
        ...
        return function handler (event, _data) {
            ...
            if (win_location.search && !data._search) {
                data._search = searchToJSON();
            }

            // append url hash to data
            if (win_location.hash && !data._hash) {
                data._hash = win_location.hash.substr(1);
            }

            // append error data to data
            if (event.error) {
                data._error = event.error;
            }

            for (i = 0; i < flowConfig.length; ++i) {
                ...
                if (config.data) {
                    var copy = JSON.parse(JSON.stringify(config.data));
                    for (key in copy) {
                        data[key] = copy[key];
                    }
                }
                ...
                if (config.add) {
                    ...
                    for (key in config.add) {

                        var add = config.add[key];
                        var match = add.match(find_tmpl);
                        ...
                        // extend data with keys from an other object
                            ...
                            _path = add.replace(find_index, '.' + (elmIndex || 0));
                ...
                if (config.load) {
                    ...
                    var match = config.load[1].match(find_tmpl);
                    ...
                    if (match) {
                        for (var m = 0, value; m < match.length; ++m) {
                            ...
                            // replace value in route
                            if (typeof value !== 'undefined') {
                                config.load[1] = config.load[1].replace(match[m], value);
                            }
                        }
                    }
                ...
                if (config.route) {
                    ...
                    var match = config.route.match(find_tmpl);
                ...
                if (config.event) {
                    eSelf.config.call(eSelf, config, event, data);
                }

                if (config.call) {

                    if (typeof config.call === 'string' && typeof (config.call = eSelf._path(config.call) || config.call) !== fn) {
                        return console.error(self._name + ':', 'Cannot call', eSelf._name + ':' + config.call);
                    }

                    // handle "then" configuration
                    if (config.then && typeof config.then !== fn) {

                        var errorHandler = config.error ? act(config.error, dom, elmIndex, elms, dontPrevent) : null;
                        var handler = act(config.then, dom, elmIndex, elms, dontPrevent);

                        // create callback handler
                        config.then = function (err, data) {
                    ...
                    // call method
                    config.call.call(eSelf, event, data, config.then || function () {});
                }
            }
    ...
    // add dynamic data to events data argument
    function gatherData (config, data) {
