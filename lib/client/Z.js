    ...
    var classes = {
        ...
        I: {
            ...
            emit: function(event) {
                ...
                args = args || self._toArray(arguments).slice(1);
        ...
        return function actHandler () {
            ...
            if (config.emit) {
                for (i = 0; i < config.emit.length; ++i) {
                    ...
                    // get item from model
                    if (emit.item && self.model && self.model[emit.item.model] && self.model[emit.item.model].data[emit.item.index || elmIndex || 0]) {
                        item = self.model[emit.item.model].data[emit.item.index || elmIndex];
                    }
                    ...
                    if (emit.route) {
                        ...
                        if (item) {
                            ...
                            for (var m = 0, value; m < match.length; ++m) {
                                ...
                                value = self._path(match[m].replace('{', '').replace('}', ''), item);
                        ...
                        // search path in item, if path is given
                        if (item) {

                            // get a value from the item
                            if (emit.item.path) {
                                item = self._path(emit.item.path, item);
                            }

                            // attach item to event object
                            if (dom) {
                                args[0].item = item;

                            // unshift item to static args
                            } else if (emit.args) {
                                emit.args.unshift(item);
