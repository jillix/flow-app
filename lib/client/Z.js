(function(modules, cache, wsCache, css, fn, win, doc) {
    ...
    var factories = {
        ...
        V: function (view, config) {
            ...
            // save observer config for later use after rendering
            if (config.O) {
                view.O = config.O;
            }
    ...
    // TODO view observe config is called automatically after rendering. so,
    // there is no need to set up view observe events after loading
    function loader (factory, clone, config, callback, sub) {
        var self = this;
        var elements = 1;
        var count = 0;
        var inits = [];
        var handler = function (err, constructor, _inits) {
            ...
            for (i = 0; i < config.actions.length; ++i) {
                ...
                if (elm) {
                    for (e = 0; e < elm.length; ++e) {
                        elm[e].addEventListener(config.actions[i].name, act.call(self, config.actions[i].A, true, e));
                    }
                }
            }
        }
    ...
    // act (handlers)
    function act (config, dom, elmIndex) {
        var self = this;
        var method;
        ...
        return function () {
            ...
            if (config.emit) {
                for (i = 0; i < config.emit.length; ++i) {
                    ...
                    self[emit.type](emit.name, emit.args);
                }
            }
