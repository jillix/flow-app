(function(modules, cache, wsCache, css, fn, win, doc) {
            ...
            for (i = 0; i < config.actions.length; ++i) {
                ...
                if (elm) {
                    for (e = 0; e < elm.length; ++e) {
                        elm[e].addEventListener(config.actions[i].name, act.call(self, config.actions[i].A, true, e, elm));
                    }
                }
            }
        }
    ...
    // act (handlers)
    function act (config, dom, elmIndex, elms) {
        var self = this;
        var method;
