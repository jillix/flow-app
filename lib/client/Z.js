    ...
    function createCommonJsModulesInOrder (moduleSources, callback) {
        ...
        for (var i = (moduleSources.length - 1), l = 0; i >= l; --i) {
            ...
            if (typeof modules[moduleSources[i]] === fn && !modules[moduleSources[i]]._eval) {
                    ...
                    key = moduleSources[i].replace(/\.[^\.]+\.js$/i, '.js');

                    // execute CommonJS module
                    modules[key] = modules[moduleSources[i]].call(module.exports, require(module), module, module.exports);
                    modules[key]._eval = true;
            }
        }

        // remove file fingerprint
        key = moduleSources[0].replace(/\.[^\.]+\.js$/i, '.js');

        // return first module of dependency list
        callback(null, modules[key] ? modules[key].exports : null);
    }
