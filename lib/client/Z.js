(function(modules, cache, wsCache, css, fn, win, doc) {
    ...
    var classes = {
        ...
        I: {
            ...
            // route to url
            // TODO on instance init route, call only route event on instance
    ...
    function act (config, dom, elmIndex, elms, dontPrevent) {
        ...
        return function actHandler (event, data) {
            var self = _self;
            var i;
            ...
            if (config.emit) {
                ...
                for (i = 0; i < config.emit.length; ++i) {
                    ...
                    if (emit.to && !(self = cache.I[emit.to])) {
                        continue;
                    }

                    // appedn static data to event data
                    if (emit.data) {
                        data = data || {};

                        // copy emit data to event data
                        for (var key in emit.data) {
                            data[key] = emit.data[key];
                        }
                    }
                    ...
                    if (emit.item) {
                        ...
                        var path = emit.item.path;
                        var prop = emit.item.prop;
                        ...
                        if (path[0] === '/') {
                            ...
                            var map_path = path.split('/').slice(1, -1);
                            ...
                            for (var p = 0, key; p < map_path.length; ++p) {
                                ...
                                if (test_tmpl.test(key) && url_path[p]) {
                                    key = key.replace(find_braces, '');

                                    // merge url data to event data
                                    prop[key] = url_path[p];
                                }
                            }

                        // replace positional operator with index number and find item
                        } else if (typeof (path = self._path(path.replace(find_index, '.' + (elmIndex || 0)))) !== 'undefined') {

                            // append found item into event data
                            data[prop || 'item'] = path;
                        }
                    }
                    ...
                    if (emit.event) {
                        self.emit.call(self, emit, event, data);
                    }

                    if (emit.call) {

                        if (typeof emit.call === 'string') {
                            emit.call = self._path(emit.call);
                        }

                        // call method
                        emit.call.call(self, event, data);
                    }
                }
            }
        };
