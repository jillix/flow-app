    ...
    var classes = {
        ...
        I: {
            ...
            route: function (url, fromPopstate) {
                ...
                var self = this;
                var url_location = win.location;
                var current = location.href.split(/^(.*:)\/\/([a-z\-.]+)(:[0-9]+)?(.*)$/)[4];

                // emit current url if url is false
                url = url || current;

                // append query or hash to pathname
                if (url[0] === '?' || url[0] === '#') {
                    url = url_location.pathname + (url[0] === '?' ? url : url_location.search) + (url[0] === '#' ? url : url_location.hash);
                }

                // dynamic urls
                // TODO check with regexp for /\/*\/|\/*/?$/
                if (url.indexOf('/*/') > -1) {
                    var path = url_location.pathname.split('/');
