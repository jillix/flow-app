(function(modules, cache, wsCache, css, fn, win, doc) {
    ...
    function initConstructors (err, constructors, callback) {
        ...
        var handler = function (init) {
            ...
            // call constructor
            init[0].call(init[1], init[2].config || {}, function (err) {

                // start observing
                if (init[2].O) {
                    observe.call(init[1], init[2].O);
                }
    ...
    function loader (factory, clone, config, callback, sub) {
        ...
        var handler = function (err, constructor, _inits) {
            ...
            // catch constructor
            if (constructor) {
                inits.push([constructor, clone, config]);
            }
        ...
        if (config.L) {
            ...
            elements += config.L.length;

            // load sub elements
            for (var i = 0; i < config.L.length; ++i) {
                self._load(config.L[i].type, config.L[i].name, handler, true);
            }
        }

        // load scripts
        if (config.scripts && config.scripts.length > 0) {
            loadJS(config.module, config.scripts, handler);
