    ...
    function createHandler (outConfig, dom, elmIndex, elms, dontPrevent) {
        ...
        return function handler (event, _data) {
            ...
            for (i = 0; i < outConfig.length; ++i) {
                ...
                if (config.set) {
                    ...
                    for (key in config.set) {
                        ...
                        // extend data with keys from an other object
                            ...
                            // update data object
                            data[key] = self._path(path) || self._path(path, event, true);
                        }
                    }
                ...
                // adapt to method
                if (config.route) {
                    eSelf.route(parsePath.call(self, config.route, data), event);
                }
                ...
                if (config.call) {
                    ...
                    if (config.then && typeof config.then !== fn) {
                        ...
                        if (config.then[0]) {
                            errorHandler = createHandler.call(eSelf, config.then[0], dom, elmIndex, elms, dontPrevent);
                        }

                        if (config.then[1]) {
                            handler = createHandler.call(eSelf, config.then[1], dom, elmIndex, elms, dontPrevent);
                        }
                        ...
                        config.then = function (err, data) {
                            ...
                            if (err && errorHandler) {
                                event.error = err;
                                errorHandler.call(eSelf, event, data);
                            } else if (handler) {
                                handler.call(eSelf, event, data);
                            }
                        }
                    }
