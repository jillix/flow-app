        ...
        return function handler (event, _data) {
            ...
            for (i = 0; i < outConfig.length; ++i) {
                ...
                if (config.call) {
                    ...
                    // handle stdOut and stdErr configuration
                    if (config.call[1] && typeof config.call[1] !== fn) {

                        var errorHandler;
                        var handler;

                        if (config.call[1]) {
                            handler = createHandler.call(self, [config.call[1]], dom, elmIndex, elms, dontPrevent);
                        }

                        if (config.call[2]) {
                            errorHandler = createHandler.call(self, [config.call[2]], dom, elmIndex, elms, dontPrevent);
                        }
                        ...
                        // create callback handler
                            ...
                            config.call[1] = function (err, data) {
                    ...
                    // call method
                    config.call[0].call(eSelf, event, data, config.call[1] || function () {});
                }
            }
