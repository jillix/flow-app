 ...
 */
engine.path = function (path, scopes, count) {
    ...
    for (var i = 0; i < path.length; ++i) {
        if ((scope = scope[path[i]]) === undefined) {
            return scopes[++count] ? engine.path(o, scopes, count) : undefined;
        }
    }
...
engine.deep = function (object) {
    ...
    for (var i = 0; i < keys.length; ++i) {
        ...
        for (var ii = 0; ii < subkeys.length; ++ii) {
            subkey = subkeys[ii];
            parentObj[subkey] = parentObj[subkey] === undefined ? (subkeys[ii + 1] === "0" ? [] : {}) : parentObj[subkey];
 ...
 */
engine.roleAccess = function (item, role) {
