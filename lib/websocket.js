...
exports.mux = function (chain, options) {

    // check client host
    options.host = options.host || (window ? window.location.host : undefined);
    if (!options.host) {
        return chain.i.emit('error', new Error('Websocket.mux: No host.', options));
    }

    // get socket
    options.socket = options.socket || sockets[options.host];
    if (!options.socket) {
        var socket;
        socket = new WebSocket('wss://' + options.host);
        sockets[options.host] = socket;
        ...
        socket.onmessage = Demux(this);
        socket._streams = {};
        options.socket = socket;
    }

    chain.i.pipe(Mux(options.socket, chain.o, options));
}
