...
function proxyRequest (host, socket, buffer, application) {
    ...
    var appSocket = net.connect(application.port, application.host);
    ...
    var server = net.createServer(function(socket) {
        ...
        socket.once('data', function (buffer) {
            ...
            startApp(host, runningApps, function (err, application) {
                ...
                proxyRequest(host, socket, buffer, runningApps[host] = application);
            });
        });
    });
    
    server.listen(M.config.port, M.config.host);
});
