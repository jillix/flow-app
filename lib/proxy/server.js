...
function server (socket, ws) {
    ...
    socket.once('data', function (buffer) {
        ...
        // is the URL to long?
        if (bfrstr.indexOf('\n') > 2048) {
            return M.proxy.send(socket, 414, 'Request-URL Too Long.\r\n' + buffer.toString('ascii'));
        }

        // get host
        var host = bfrstr.match(/host\: *([a-z0-9\-_\.]+)(:[0-9]{2,5})?/i);
        if (!host) {
            return M.proxy.send(socket, 400, 'No Host found in headers.\r\n' + buffer.toString('ascii'));
        }
        
        host = host[1];
        
        if (M.apps.cache[host] === null) {
            return M.proxy.send(socket, 503, 'Application is starting...');
        }
        ...
        M.spawner.startApp(host, function (err, application) {
            ...
            if (err) {
                ...
                return M.proxy.send(socket, statusCode, err.message);
            }
