...
function server (socket, ws) {
    ...
    socket.once('data', function (buffer) {
        ...
        if (M.cache.apps.cache[host]) {
            return M.api.proxy.pipe(host, socket, buffer, ws);
        }
        ...
        startApp(host, function (err, application) {
            ...
            M.api.proxy.pipe(host, socket, buffer, ws);
        });
    });
...
process.on('SIGTERM', M.api.proxy.killEmAll);
process.on('SIGINT', M.api.proxy.killEmAll);
process.on('uncaughtException', M.api.proxy.killEmAll);
