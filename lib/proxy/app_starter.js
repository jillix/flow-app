...
function startApplication(host, socket, buffer, runningApps, callback) {
    
    M.model.getDomainApplication(host, false, function(err, application) {
        
        if (err) {
            runningApps[host] = undefined;
            return callback(host, socket, buffer, [500, err.toString()]);
        }
        
        if (!application) {
            runningApps[host] = undefined;
            return callback(host, socket, buffer, [404, 'Application not found.']);
        }
        
        if (!application.appId) {
            return callback(host, socket, buffer, [500, 'No application ID found.']);
        }
        ...
        if (!fs.existsSync(appPath)) {
            return callback(host, socket, buffer, [500, "Application directory not found: " + application.appId]);
        }
        ...
        getPid(application.appId, function (pid) {
            ...
            if (pid) {
                ...
                return getPort(pid, function () {
                    ...
                    if (!port) {
                        return callback(host, socket, buffer, [500, 'No port found for running application: ' + application.appId]);
                    }
            ...
            getFreePort(function (freePort) {
                ...
                if (!freePort) {
                    return callback(host, socket, buffer, [500, 'No free port found for application: ' + application.appId]);
                }
                ...
                app.stdout.once('data', function (data) {
                    ...
                    if (data.toString('ascii') !== application.appId) {
                        callback(host, socket, buffer, [500, 'App spawn response error: ' + data.toString('ascii')]);
                    }
