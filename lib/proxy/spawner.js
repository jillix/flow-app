...
function startApplication(host, runningApps, callback) {
    ...
    M.app.getFromHost(host, {
        ...
        // otherwise the piped streams below will crash the mono proxy server
        if (!fs.existsSync(appPath)) {
            runningApps[host] = undefined;
            return callback([500, "Application directory not found: " + application.id]);
        }
        ...
        getPid(application.id, function (pid) {
            ...
            application = {
                ...
                publicRole: application.publicRole ? parseInt(application.publicRole.split(':')[1], 10) : 1,
            ...
            if (pid) {
                ...
                return getPort(pid, function (port) {
                    ...
                    if (!port) {
                        runningApps[host] = undefined;
                        return callback([500, 'No port found for running application: ' + application.id]);
                    }
            ...
            getFreePort(function (freePort) {
                ...
                if (!freePort) {
                    runningApps[host] = undefined;
                    return callback([500, 'No free port found for application: ' + application.id]);
                }
                
                var log = fs.createWriteStream(appPath + "/log.txt");
                var app = spawn('node', [
                    M.config.root + '/lib/application/server.js',
                    '--app', JSON.stringify(application),
                    '--port', freePort,
