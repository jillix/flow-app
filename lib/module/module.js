    ...
    self[link['1'] ? 'once' : 'on'](link.event, function (err, data) {
        ...
        if (link.call) {
            ...
            if (typeof link.call === 'string') {
                link.call = object.path(link.call, self) || link.call;
            }

            if (typeof link.call !== 'function') {

                err = new Error('links: Cannot find module method: ' + link.call);

                // websocket requests
                if (self.link.ws) {
                    sendHandler(self._name, self)(err);

                // http requests
                } else {
                    sendHandler(self._name, self)(500, err);
                }
...
function instanceFactory (name, module, compInst, callback) {
    ...
    instance._client.scripts = module.dependencies ? module.dependencies.concat(compInst.scripts || []) : [];

    // TODO better do callback buffering on instances, then copy the scripts
    instance._scripts = instance._client.scripts.slice();
