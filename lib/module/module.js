...
// init caches
var pojoInstances = cache.pojo('instances', true);
...
function loadinstance (name, role, callback) {
    ...
    compInstances.get(name, role, function (err, instance) {
...
function moduleFiles () {
    ...
    if (self.link.pathname.replace(/[^a-z0-9\/\.\-_@]|\.\.\//gi, '') !== self.link.pathname) {
        self.link.res.writeHead(404, {'content-type': 'text/plain'});
        return self.link.res.end('Incorrect data in module request URL');
    }
