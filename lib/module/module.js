...
function instanceFactory (name, module, dbInstance, callback) {
    ...
    if (dbInstance.send) {
        ...
        dbInstance.send.up.push(env.Z_SEND_MODULE_REQ);
        dbInstance.send.down.push(env.Z_SEND_MODULE_RES);
        dbInstance.send.up.push(env.Z_SEND_MODULE_DATA_REQ);
        dbInstance.send.down.push(env.Z_SEND_MODULE_DATA_RES);
...
function loadinstance (name, role, callback) {
    ...
    compInstances.set(name, role, function (err, instance) {
        ...
        // check access
        if (!instance.roles['*'] && !instance.roles[role]) {
...
function getCachedInstance (instance, role) {
