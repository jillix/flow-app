...
function getCachedInstance (session, instance, event) {
    ...
    if (instance && (instance.Z.name === 'Z' || instance.Z.roles[session[M.config.session.role]]) && instance.Z.access[event]) {
        return instance;
    }
    return null;
}
...
middleware(function (err, mware) {
    ...
    // start http server
    M.http = http.createServer(mware(requestHandler));

    // start ws server
    M.ws = new WebSocketServer({server: M.http});
    M.ws.on('connection',  mware(connectionHandler));
