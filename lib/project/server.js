...
var instanceCache = cache('instances');

// check if instance exists role and event access
function getCachedInstance (session, instance, event) {

    instance = instanceCache.get(instance);

    if (instance && (instance.Z.name === 'Z' || instance.Z.roles[session[env.Z_SESSION_ROLE_KEY]]) && instance.Z.access[event]) {
        return instance;
    }
    return null;
}
...
function requestHandler (err, req, res) {
    ...
    if (path[0] === env.Z_OP_KEY) {
        ...
        var instance = getCachedInstance(req.session, path[1], path[2]);
...
function connectionHandler (err, ws) {
    ...
    ws.on('message', function (data) {
        ...
        var instance = getCachedInstance(ws.session, data[0][0], data[0][1]);
...
middleware(function (err, mware) {
    ...
    // start http server
    var http = http.createServer(mware(requestHandler));
    ...
    // start http server
    http.listen(env.Z_PORT, env.Z_HOST);
});
