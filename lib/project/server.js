...
function requestHandler (err, session, req, res) {
    ...
    if (path[0] === env.Z_OP_KEY) {
        ...
        if (instance) {
            ...
            operation.link[env.Z_SESSION_ROLE_KEY] = role;
            operation.link[env.Z_SESSION_USER_KEY] = session[env.Z_SESSION_USER_KEY];
            operation.link[env.Z_SESSION_LOCALE_KEY] = session[env.Z_SESSION_LOCALE_KEY];

            // set a empty response header object
            operation.link.res.headers = {};

            // emit operation
            operation.emit(path[2], req, res);
            req.resume();
...
function connectionHandler (err, session, ws) {
    ...
    ws.on('message', function (data) {
        ...
        if (instance) {
            ...
            // save session in link
            message.link[env.Z_SESSION_ROLE_KEY] = role;
            message.link[env.Z_SESSION_USER_KEY] = session[env.Z_SESSION_USER_KEY];
            message.link[env.Z_SESSION_LOCALE_KEY] = session[env.Z_SESSION_LOCALE_KEY];
