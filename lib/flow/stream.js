...
exports.Event = function (options) {
    ...
    sequence._write = function (chunk, enc, next) {
        ...
        if (!sequence.seq) {
            return sequence.out.end(new Error('No handlers for event.'));
        }
    ...
    sequence.next = function (err, data) {
        ...
        // get handler
        var handler = sequence.seq[sequence._err ? 'error' : 'data'][++sequence._pos];
        if (!handler) {
            ...
            if (sequence.linked) {
                ...
                if (sequence.seq.type === '|') {
                    sequence.push(data);
                }

                // handle back pressure
                if (!sequence.linked.write(data)) {
                    sequence.linked.once('drain', sequence._next.bind(sequence));
                    return;
                }
        ...
        handler[1].call(handler[0], sequence.next.bind(sequence), handler[2], data);
    };
