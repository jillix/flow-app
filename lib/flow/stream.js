...
// writable
exports.Event = function (stdin, stdout, endHandler) {
    
    var eventStream = Stream.Duplex();

    eventStream._write = function (chunk, enc, callback) {
        // call all data handlers
        // write data handlers result to writable target stream (stdout, stderr)
        //this.events = {};
        console.log('FLOW EVENT _write:', chunk, enc);
        this.push(chunk);
        callback();
    }

    eventStream._read = function (n) {
        // use data handler result as source
        // this.push(dataHandlerResult);
        console.log('FLOW EVENT _read:', n);
    }
