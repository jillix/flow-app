...
exports.net = function (req, res, next) {

    var app = req.app;

    // TODO parse the url and emit on flow

    (res || req).send('Trucken');

    if (typeof next === 'function') {
        next();
    }
    // TODO set up stream on socket
    /*
    // serve the client
...
exports.emit = function (event) {
    ...
    console.log('Flow emit:', event);
    return {write: function (err, data) {
...
exports.start = function (name, callback) {
    ...
    // init the flow config
    var stream = engine.flow('@/IC');
    stream.data(function (err, instance) {

        stream.end();

        if (err) {
            return engine.flow('instanceLoadError').write(err);
        }

        if (!instance) {
            var nc_stream = engine.flow('@/I');
            nc_stream.data(function (err, instance) {
