        ...
        */
        ...
        CoreInst.load(options.to, options.session, function (err, instance) {
            ...
            getEvent(instance, options, function (err, flowEvent) {
                ...
                eventStream.seq = flowEvent.start;

                // setup sub streams (sequences)
                console.log(flowEvent);

                if (flowEvent.seqs && flowEvent.seqs.length) {

                    var prevStream = eventStream;
                    flowEvent.sections.forEach(function (section, index) {

                        console.log('\n\nSECTION ' + index + ':\n', section);

                        // call flow or stream handler
                        let linked;
                        let shOptions = Object.create(section.link[2]);
                        shOptions.session = options.session;

                        if (typeof section.link[1] === 'string') {
                            linked = section.link[0].flow(section.link[1], shOptions); 
                        } else {
                            linked = section.link[1].call(section.link[1], prevStream, shOptions) || {};
                        }

                        // attach linked stream to input stream
                        if (typeof linked._write === 'function') {
                        ...
                        if (typeof linked.pipe === 'function') {
