...
var FlowEmitter = {
    ...
    flow: function (eventName,  options, callback) {
        ...
        */
        ...
        console.log('CORK', options.to, options.emit);
        eventStream.cork();
            ...
            getEvent(instance, options, function (err, flowEvent) {
                ...
                if (flowEvent[1]) {
                    eventStream.on('error', function (error) {
                        ...
                        console.log('EVENT STREAM "' + options.to + '/' + options.emit + '":', error);
                        //eventStream.error.end(error);
                    });
                }

                console.log('UNCORK', instance._name, options.emit);
                eventStream.uncork();
            });
        });
