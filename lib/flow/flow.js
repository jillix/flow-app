    ...
    flow: function (stream, options) {
        ...
        */
        ...
        if (!utils.access(instance, stream.session)) {
            if (options.err) {
                options.err.end(new Error('Flow: Access denied to instance "' + instance._name + '".'));
            }
            return;
        }
        ...
        if (events[eventName] && events[eventName].length) {
            ...
            getEvent(instance, eventName, options, stream.session, function (err, flowEvent) {

                if (err) {
                    return eventStream.stderr.end(err.toString());
                }
...
function getEvent (instance, eventName, options, session, callback) {

    instance = options.to || (eventName.indexOf('/') > 2 ? eventName.split('/')[1] : instance._name);
    process.nextTick(CoreInst.load.bind(CoreInst), instance, session, function (err, instance) {
        ...
        if (!instance._flow[eventName]) {
            return callback(new Error('Flow.getEvent: Event "' + eventName + '" not found on instance "' + instance._name+ '".'));
        }
