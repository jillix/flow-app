...
function _factory (name, session, callback) {
    ...
    flow.composition(name, function (err, composition) {
        ...
        if (!composition || !composition.module) {
            return callback(new Error('Invalid composition'));
        }

        if (!utils.access(composition, session)) {
            return callback(new Error('Access denied for instance:' + composition.name));
        }
        ...
        var readHandler = function (err, cjsMod) {
            ...
            if (--count === 0) {
                build.call(flow, module, composition, callback, (name === '*'));
            }
        };
...
function build (module, config, callback, isEntrypoint) {
    ...
    if (config.flow) {
        config.flow.forEach(function (flow) {
            if (flow.constructor === Array) {
                ...
                instance._flow[flow[0]] = flow.slice(1);
            }
        });
    }
    ...
    callbacks(null, instance, cbBuffer[isEntrypoint ? '*' : instance._name]);
}
