 ...
 */
exports.eventAccess = function (role, module_instance, event) {

    // get instance with role check
    module_instance = engine.moduleInstance.get(module_instance, role);
 ...
 */
exports.factory = function (composition, callback) {
    ...
    if (composition.main) {
        ...
        try {
            ...
            var path = composition.main[0] === '/' ? engine.repo : engine.paths.app_modules + composition.module + '/';
...
function load (link) {
    ...
    link.data(function (err, module_instance_name) {
        ...
        engine.moduleInstance.create(module_instance_name, link.role, function (err, module_instance) {

            // handle error
            if (err || !module_instance) {
                err = 'Module instance "' + module_instance_name + '" ' + err || 'not found.';
                console.error('lib/module.js#178', err.toString());
                return link.end(err);
            }
