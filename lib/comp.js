...
function handleModule (moduleName, modulePackage, callback) {
    ...
    if (modulePackage.clientDependencies) {
        ...
        for (var i = 0, dependency; i < modulePackage.clientDependencies.length; ++i) {
            ...
            getModulePackage(dependency, loadHandler(i, modulePackage, callback), modulePackage);
        }
    }
...
function loadHandler (index, parentPackage, callback) {

    return function (err, modulePackage) {
        console.log('load handler', err, modulePackage);
    }
}
