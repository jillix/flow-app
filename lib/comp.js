...
module.exports = function (name, role, callback) {
    ...
    fs.readJson(path, function (err, composition) {
        ...
        getModulePackage(composition.module, function (err, modulePackage) {
            ...
            if (err) {
                console.error('lib/comp.js#73', err.toString(), composition.module);
                return callback(err);
            }
...
console.log('MODULE CACHE KEYS:', Object.keys(engine.modules._data));
        ...
        if (err || !modulePackage) {
            ...
            if (modulePackage.components) {
                ...
                for (var type in modulePackage.components) {
                    ...
                    // ensure component type array
                    parentPackage && (parentPackage.components[type] = parentPackage.components[type] || []);

                    // push the module components to the parent module components
                    for (var i = 0; i < modulePackage.components[type].length; ++i) {

                        // append module version to client path
                        if (modulePackage.components[type][i][0] !== '/') {
                            modulePackage.components[type][i] = modulePackage.name + '@' + modulePackage.version + '/' + modulePackage.components[type][i];
                        }

                        parentPackage && parentPackage.components[type].push(modulePackage.components[type][i]);
                    }
                }
            }
        }
...
function updateComponentPaths (components, options, callback) {
    ...
    for (var type in components) {
        ...
        engine.file.addFingerprints(parentDir, components[type], function (err, paths) {
            ...
            if (err) {
                console.error('lib/comp.js#358', err.toString());
                return callback(err);
            }
