        ...
        getModulePackage(composition.module, function (err, modulePackage) {
            ...
            updateComponentPaths(
                ...
                function (err, components) {
                    ...
                    console.log(composition.client.scripts);
                    console.log(composition.client.styles);
                    console.log(composition.client.markup);
                    console.log('-----------------------------------------------');
...
function getModulePackage (module, callback, parentPackage) {
    ...
    if (modulePackage) {
        console.log('module', moduleName, 'loaded from module cache')
...
function loadHandler (parentPackage, callback) {
    ...
    return function (err, modulePackage) {
        ...
        if (err || !modulePackage) {
            ...
            parentPackage && --parentPackage._elmsToLoad === 0;
        ...
        if (!parentPackage) {
            return callback(err, parentPackage || modulePackage);
        }
...
function updateComponentPaths (components, options, callback) {
    ...
    for (var type in components) {
        ...
        engine.file.addFingerprints(parentDir, components[type], function (err, paths) {
            ...
            if (err) {
                console.error('lib/comp.js#342', err.toString());
                return callback(err);
            }
