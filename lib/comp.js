        ...
        fs.readJson(path, function (err, composition) {
            ...
            getPackackeInfo.call(self, composition.module, function (err, moduleConfig) {
                ...
                if (moduleConfig) {
                    ...
                    if (moduleConfig.clientDependencies) {
                        composition.client.scripts = moduleConfig.clientDependencies;
                    }
...
function getPackackeInfo (module, callback) {
    ...
    if (typeof module === 'object') {
        ...
        if (!module.clientDependencies) {
            return callback(null, module);
        }

        // add fingerprints to custom module dependencies
        // TODO check if finterprints are already added
        fingerprint.addToFiles(module, module.clientDependencies, function (err, scripts) {
            ...
            module.clientDependencies = scripts;
    ...
    fs.readJson(path, function (err, packageInfo) {
        ...
        // handle errors
        if (err || !packageInfo) {
            return callback(err || new Error('No module package found.'));
        }
        ...
        if (!packageInfo.clientDependencies) {
            return callback(null, self._modules[module]);
        }

        // add fingerprints to client side dependencies
        fingerprint.addToFiles(module, packageInfo.clientDependencies, function (err, scripts) {
            ...
            // update the module dependencies with new script paths
            self._modules[module].clientDependencies = scripts;
