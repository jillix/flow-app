...
function cleanupApplicationDependencies(descriptor, callback) {
    ...
    dir.removeDirectory(M.config.APPLICATION_ROOT + descriptor.appId + "/mono_modules", function(err) {

        if (err) { return callback; }

        dir.removeDirectory(M.config.APPLICATION_ROOT + descriptor.appId + "/node_modules", callback);
    });
}
...
function copyApplicationDependencies(descriptor, dependencies, callback) {
    ...
    function copyApplicationDependenciesSequential(index) {
        ...
        dir.copyDirectory(modulePath, appModuleRoot + module.getModulePath(), { createParents: true }, function(err) {
