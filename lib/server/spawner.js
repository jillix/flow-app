...
function getFromHost (host, callback) {
    ...
    M.db.applications.findOne({domains: host}, {fields: {process: 1, uid: 1, gid: 1, owner: 1}}, function (err, data) {
...
function getFreePort (callback) {
    ...
    child.on('exit', function (code) {
        ...
        for (var i = M.config.appPortStart, l = M.config.appPortEnd; i < l; ++i) {
            if (!usedPorts[i]) {
                freePort = i;
                break;
            }
    ...
    getFromHost(host, function(err, dbApplication) {
        ...
        getPid(application.id, function (err, pid) {
            ...
            getFreePort(function (freePort) {
                ...
                var spawnOptions = {
                    cwd: M.config.paths.USERS_ROOT + dbApplication.owner + '/apps/' + application.id + '/',
