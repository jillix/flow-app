...
module.exports = function handler (session, req, res) {
    ...
    if (path[0] === engine.operation_id) {
        ...
        if (path[1] === engine.operation_id) {
            ...
            if (link.path[0] === engine.public_file_id) {
                link.path = link.path.slice(1);
                link.name = 'extFile';
    ...
    if (path[0] === engine.public_file_id) {
        ...
        engine.static.public(url.pathname.substr(3), req, res);
        return req.resume();
    }

    // return the engine client
    engine.static.client(url.pathname, req, res);
    req.resume();
};
