...
module.exports = function handler (session, req, res) {
    ...
    if (!Instance.eventAccess(instance, session[engine.session_role], eventName)) {
        ...
        return end(404, 'Operation not found.');
    }
    ...
    // get a duplex stream and setup flow
    // ..if an id is sent, get stream from cache?
    var stream = instance.flow(eventName, context);
    ...
    // write event stream data to response
    stream.data(function (err, data) {
        send(res, data, context.headers);
    });
    
    // ..overwrite end function?
    stream.end = function (code, headers, data) {
        end(code, context.headers, data);
    };
...
function send (response, data, headers) {
