...
function install (module, callback) {
    ...
    if (fs.existsSync(M.config.MODULE_ROOT + module.getVersionPath())) {
        ...
        if (!module.version.match(DEV_MODULE_REGEX)) {
        ...
        // if the "dev" version, always reinstall this module
            ...
            // first uninstall the dev module
    ...
    fetch(module, local, function(err, isNew) {
        ...
        readModuleDescriptor(module, function(err, descriptor) {
            ...
            if (isNew) {
                ...
                // alter dependencies
                if (M.config.compressFiles && descriptor.dependencies) {
                    var newDeps = [];
                    var virtualDeps = [];
                    
                    for (var i = 0, l = descriptor.dependencies.length; i < l; ++i) {
                        
                        if (i > 0 && descriptor.dependencies[i][0] === '/' && descriptor.dependencies[i][1] !== '/') {
                            
                            var path = module.getVersionPath().split('/');
                            if (path[path.length - 1].indexOf('dev_') === 0) {
                                path[path.length - 1] = 'dev';
                            }
                            
                            virtualDeps.push('#' + path.join('/') + descriptor.dependencies[i]);
                            continue;
                        }
                    
                        newDeps.push(descriptor.dependencies[i]);
                    }
                    
                    descriptor.dependencies = newDeps;
                }
            }
            ...
            installDependencies(module, descriptor, function(err, installedDependencies, scripts) {
                ...
                if (M.config.compressFiles) {
                    virtualDeps.unshift(scripts.shift());
                    scripts = virtualDeps.concat(scripts);
                }
...
function wrapClientScripts (module, descriptor, callback) {
    ...
    mod_client.wrapScripts(module, scripts);
}
...
function installDependencies (module, descriptor, callback) {
    ...
    for (var i in descriptor.dependencies) {
        ...
        // ignore virtual depedencies
        if (descriptor.dependencies[i][0] === '#') {
            continue;
        }
        var dep = descriptor.dependencies[i];
        ...
        if (dep[0] === '/') {
            ...
            // it is an internal script, so write it in the SONV form
                ...
                scripts.push([module.getModulePath() + '/' + version + dep]);
            }
        }
...
function fetch (module, local, options, callback) {
    ...
    // if this version is already present give up
    if (fs.existsSync(M.config.MODULE_ROOT + module.getVersionPath())) {
    ...
    addDirectory(module.source, module.owner, module.name, function(err) {
        ...
        // TODO this will become obsolete on the server
        if (local) {
        ...
        // else clone from the web
            ...
            cloneVersion(module, options, modifiedCallback);
        }
    });
}
...
function cloneVersion (module, options, callback) {
    ...
    if (M.config.log.moduleInstallation || M.config.logLevel === 'verbose') {
        console.log('Cloning module from ' + url + ' into ' + dirName + '/' + module.version);
    }
    ...
    M.repo.cloneToDir(url, dirName, module.version, cloneOptions, function(err) {

        if (err) {
            if (err.code === 'API_REPO_NOT_FOUND_FOR_URL') {
                err = M.error(M.error.API_MOD_NOT_FOUND, module.getModulePath());
            }
            return callback(err)
        };

        var tag = module.version.match(DEV_MODULE_REGEX) ? M.config.MODULE_DEV_TAG : module.version;

        // reset to this version
        M.repo.checkoutTag(dirName + '/' + module.version, tag, function(err) {

            if (err) {
                if (err.code === 'API_REPO_TAG_NOT_FOUND') {
                    err = M.error(M.error.API_MOD_VERSION_NOT_FOUND, module.getVersionPath());
                }
                return callback(err)
            };
