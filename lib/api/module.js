...
function uninstall (module, callback) {
    ...
    mod_model.deleteVersion(module, function(err) {
        ...
        // in case of an error, do not stop, only print the error and proceed
        if (err) {
            console.error(err);
            console.error('But continuing with module removal...');
        }
...
function installDependencies (module, descriptor, callback) {
    ...
    if (!descriptor.dependencies || !descriptor.dependencies.length) {
        return callback('A module must have at least it\'s main script as dependency');
    }
...
function addDependencyLinks (module, descriptor, installedDependencies, callback) {
    ...
    // TODO why does this come as a non-object?
    if (typeof installedDependencies !== 'object') {
        return callback(null);
    }
