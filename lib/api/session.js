...
function updateSession ( sessionID, doc, callback ){
	...
	M.mongo.connect(M.config.mongoDB.name, colName, function( err, db ){
...
function get (link, callback) {
    ...
    if (cookies.sid) {
        ...
        return getSession(cookie.sid, expire(), expire(expire_time), function(err, session) {
...
// start new Session
function start (rid, uid, locale, data, callback) {

    var session = {
        '_sid': M.util.uid(23), // generate session id
        '_exp': expire(expire_time),
        '_uid': uid,
        '_loc': locale
    };
