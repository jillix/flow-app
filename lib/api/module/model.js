...
function getConfig (miid, roleId, callback) {
    ...
    var className = 'VRole';
    var vuCluster = M.orient.getClusterByClass(className);

    if (!vuCluster) {
        return callback(M.error(M.error.DB_ORIENT_NO_CLUSTER_FOR_CLASS, className));
    }
    ...
    var command =
        'SELECT ' +
            ...
            'in.scripts AS scripts, ' +
            'config ' +
        'FROM ' +
            '(TRAVERSE ' + className + '.out FROM #' + vuCluster.id + ':' + roleId + ') ' +
        'WHERE ' +
    ...
    M.orient.sqlCommand(command, function(err, results) {
        ...
        if (results.length == 0) {
            return callback(M.error(M.error.API_MOD_NOT_FOUND, miid));
        }

        // if there are too many results
        // this is an internal error not an API since application desctiptor only allows unique miid's
        if (results.length > 1) {
            return callback(M.error(M.error.DB_ORIENT_NOT_UNIQUE, 'EUsesInstanceOf', 'role = ' + roleId + '; miid = "' + miid + '"'));
        }

        var module = results[0];

        if (!module.source || !module.owner || !module.name || !module.version) {
            return callback(M.error(M.error.DB_ORIENT_INVALID_RECORD, 'module', JSON.stringify(module)));
        }

        // append the miid scripts (defined by the application) to the end of the array
        // this way they will be loaded first
        if (module.config && module.config.scripts) {
            module.scripts = module.scripts.concat(module.config.scripts);
        }
