...
function getConfig (miid, roleId, callback) {
    ...
    colMiids.findOne(queryMiid, {fields: {_id:0, config:1, module:1, version: 1}}, function (err, miid) {
        ...
        colMods.findOne(queryMod, {fields: {_id: 0, name: 1, owner: 1, source: 1, scripts: 1}}, function (err, module) {
            ...
            if (miid.config && miid.config.scripts) {
                miid.config.scripts = module.scripts.concat(miid.config.scripts);
            }

            var config = {
                path: module.source + '/' + module.owner + '/' + module.name + '/' + miid.version,
                config: miid.config
            };
