    ...
    M.orient.connect(function(err) {
        ...
        app_model.addApplication(
            ...
            if ((M.config.log.applicationInstallation || M.config.logLevel === "verbose") && descriptor.datasources) {
                ...
                console.log(JSON.stringify(descriptor.datasources, null, 4));
            }

            installDatasources(descriptor, function(err, datasources) {
                if (err) {
                    console.error("Failled to install datasources for " + descriptor.name + " (appId: " + descriptor.appId + ")");
...
/**
 * Adds the datasources for the application in the descriptor.
 */
function installDatasources (descriptor, callback) {
    ...
    ds_model.addDatasources(descriptor._id, descriptor.datasources, callback);
}
