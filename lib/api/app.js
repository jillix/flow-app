...
var readDirRecursive = require('./fs').readDirRecursive;
...
function getPath(appId) {
    ...
    if (!appId && !M.config.app) {
        throw Error('This function must be called with an appId if you are not in the context of an application');
    }

    appId = appId || M.config.app.id;
    return M.config.APPLICATION_ROOT + appId;
}
 ...
 * directory.
 *
 * The application must have a valid descriptor file in order to be considered a
 * valid Mono application.
 */
function fetch (url, callback) {

    var son = M.repo.extractSonFromRepoFromUrl(url);
    ...
    M.repo.getJsonFromRepo(son.source, data, function(err, searchedData, descriptor) {

        if (err) { return callback(err); }

        // clone the app to the application directory
        if (M.config.log.applicationInstallation || M.config.logLevel === 'verbose') {
            console.log('Cloning application from ' + url + ' into ' + getPath(descriptor.appId));
        }
        ...
        M.repo.cloneToDir(url, M.config.APPLICATION_ROOT, descriptor.appId, cloneOptions, function(err) {

            if (err) { return callback(err); }
            
            // compress public application files
            if (M.config.compressFiles) {
                
                readDirRecursive(getPath(descriptor.appId) + '/' + (descriptor.publicDir || 'public'), function (err, files) {
                    
                    if (err) { return callback(err); }
                    
                    compress(files, function (err) {
...
function remove (appId, callback) {
    ...
    var descriptorPath = getPath(appId) + '/' + M.config.APPLICATION_DESCRIPTOR_NAME;

    uninstallFromFile(descriptorPath, function(err) {

        if (err) { return callback(err); }

        M.fs.removeDirectory(getPath(appId), callback, function (err) {
...
function getGitUrl (appId, callback) {
    M.repo.getOriginUrl(getPath(appId), callback);
}

function canUpdate (appId, callback) {

    M.repo.getLocalHeadCommit(getPath(appId), function(err, localVersion) {
...
function redeploy (appId, callback) {
    ...
    uninstallFromFile(descriptorPath, function(err) {
        ...
        cleanupApplicationDirectory(appId, function(err) {
...
function validateDescriptor(descriptor, callback) {

    // TODO validate the descriptor
    // and return callback(M.error(M.error.API_APP_DESCRIPTOR_VALIDATION, descriptor.appId, descriptor.name, 'message'));

    // scope the dev modules for this application
    var re = new RegExp('\/' + M.config.MODULE_DEV_TAG + '$');
    for (var i in descriptor.miids) {
        descriptor.miids[i].module = descriptor.miids[i].module.replace(re, '/' + M.config.MODULE_DEV_TAG + '_' + descriptor.appId);
    }
...
function installFromObject (descriptor, callback) {
    ...
    // this will verify the correctnes of the descriptor and adjust what can be adjusted
    validateDescriptor(descriptor, function(err, descriptor) {
...
/**
 * Stop an application given its application ID.
 * This will get the process id and kill it.
 * The callback has two parameters an error and a boolean that tells if the application was stopped.
 */
function stop (id, callback) {

    //this will get the process id.
    getPid(id, function (err, pid) {

        if (err) { 
            return callback(err, false); 
        }
        
        //if the application was found, kill it. If not, it means that the application is already stopped.
        if (pid) {
            process.kill(pid);
            callback(null, true);
        } else {
...
function uninstallFromObject (descriptor, callback) {
    ...
    validateDescriptor(descriptor, function(err, descriptor) {
        ...
        stop(descriptor.appId, function (err, wasStopped) {

            if (err) {
                console.error("Failled to stop application " + descriptor.appId + " (" + descriptor.name + ")");
                console.error(err);
            }
        });
...
function roleModuleUsage (descriptor, callback) {
    ...
    function roleModuleUsageSequential(miidIndex) {
        ...
        if (M.config.log.applicationInstallation || M.config.logLevel === "verbose") {
            console.log("- configuring permissions for module: " + miid);
        }
...
function addModuleInstanceOperations(descriptor, miid, miidObj, callback) {
    ...
    var mvid = descriptor.dependencies[miidObj.module];
    ...
    function addModuleInstanceOperationsSequential(index) {
        ...
        ops_model.addCanPerform(mvid, moduleData, canPerform.role, canPerform.name, canPerform.params, function(err, id) {
...
/**
 * Removes the Mono modules directory from an application.
 */
function cleanupApplicationDirectory(appId, callback) {

    if (M.config.log.applicationInstallation || M.config.logLevel === "verbose") {
        console.log("Cleaning up application: " + appId);
    }
    M.fs.removeDirectory(getPath(appId) + "/" + M.config.APPLICATION_MODULE_DIR_NAME, callback);
}
...
function installDependencies(descriptor, callback) {
    ...
    function installDependenciesSequential(index) {
        ...
        M.module.install(module, function(err, installedDependencies) {
            ...
            // TODO make sure install adds module._vid to the module
            if (err) { return callback(err); }
            ...
            // TODO move this out and copy the application dependencies only once at the end
            //      but do we have to copy them if there is an error since the app will
...
function copyApplicationDependencies (descriptor, dependencies, callback) {
    ...
    var appModuleRoot = getPath(descriptor.appId) + "/" + M.config.APPLICATION_MODULE_DIR_NAME + "/";
    var depKeys = Object.keys(dependencies);
    var count = depKeys.length;
    var errors = [];
    var index = 0;
...
function readDescriptor(path, callback) {
    ...
    M.fs.readJsonFile(path, function(err, data) {
