...
function cleanupApplicationDependencies(descriptor, callback) {
    ...
    M.fs.removeDirectory(M.config.APPLICATION_ROOT + descriptor.appId + "/" + M.config.APPLICATION_MODULE_DIR_NAME, callback);
}
...
function copyApplicationDependencies (descriptor, dependencies, callback) {
    ...
    var appModuleRoot = M.config.APPLICATION_ROOT + descriptor.appId + "/" + M.configured.APPLICATION_MODULE_DIR_NAME + "/";
    var depKeys = Object.keys(dependencies);
    var count = depKeys.length;
    var errors = [];
    var index = 0;
