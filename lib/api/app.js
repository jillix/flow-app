...
function fetch (url, callback) {
    ...
    M.repo.getJsonFromRepo(url, M.config.APPLICATION_DESCRIPTOR_NAME, function(err, descriptor) {
        ...
        if (M.config.log.applicationInstallation || M.config.logLevel === 'verbose') {
            console.log('Cloning application from ' + url + ' into ' + M.config.APPLICATION_ROOT + descriptor.appId);
        }
        M.repo.cloneToDir(url, M.config.APPLICATION_ROOT, descriptor.appId, { depth: 1 }, function(err) {
            ...
            if (M.config.compressFiles) {
                ...
                readDirRecursive(M.config.APPLICATION_ROOT + descriptor.appId + '/' + (descriptor.publicDir || 'public'), function (err, files) {
...
function remove (appId, callback) {
    ...
    var descriptorPath = M.config.APPLICATION_ROOT + appId + '/' + M.config.APPLICATION_DESCRIPTOR_NAME;
...
function cleanupApplicationDirectory(appId, callback) {
    ...
    M.fs.removeDirectory(M.config.APPLICATION_ROOT + appId + "/" + M.config.APPLICATION_MODULE_DIR_NAME, callback);
}
...
function copyApplicationDependencies (descriptor, dependencies, callback) {
    ...
    var appModuleRoot = M.config.APPLICATION_ROOT + descriptor.appId + "/" + M.config.APPLICATION_MODULE_DIR_NAME + "/";
    var depKeys = Object.keys(dependencies);
    var count = depKeys.length;
    var errors = [];
    var index = 0;
