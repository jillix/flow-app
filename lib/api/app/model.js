    ...
    translateAppId(appId, function(err, id) {
        ...
        var command =
            "SELECT " +
                "* " +
            "FROM " +
                M.orient.ridFromId("VApplication", id);
        ...
        M.orient.sqlCommand(command, function(err, results) {
            ...
            // this is an internal error not an API since the database
            // should enforce uniqueness of the application IDs
            if (results.length > 1) {
                return callback(M.error(M.error.APP_NOT_UNIQUE, appId));
            }
...
function translateAppId (appId, callback) {
    ...
    M.orient.sqlCommand(command, function(err, results) {
        ...
        if (!application || !application.rid) {
            return callback(M.error(M.error.API_APP_INVALID, appId, "missing application internal ID"));
        }
...
function getFromHost (host, fields, callback) {
    ...
    M.orient.sqlCommand(command, function (err, data) {
        ...
        if (data.length > 1) {
            return callback(M.error(M.error.APP_NOT_UNIQUE, 'domain = "' + host + '"'));
        }
