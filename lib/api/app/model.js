...
function getApplication (appId, callback) {
    ...
    translateAppId(appId, function(err, id) {
        ...
        var className = 'VApplication';

        var command =
            'SELECT ' +
                '* ' +
            'FROM ' +
                M.orient.ridFromId(className, id);
        ...
        M.orient.sqlCommand(command, function(err, results) {
            ...
            if (!results || results.length == 0 || !results[0]) {
                return callback(M.error(M.error.API_APP_NO, appId));
            }

            // if there are too many results
            // in case the database does not the enforce uniqueness of the application IDs
            if (results.length > 1) {
                return callback(M.error(M.error.DB_ORIENT_NOT_UNIQUE, className, 'appId = "' + appId + '"'));
            }
...
function translateAppId (appId, callback) {
    ...
    M.orient.sqlCommand(command, function(err, results) {
        ...
        // NOTE results.length > 1 should never happen because there isa unique index on application.id

        var application = results[0];

        // if the application does not have the required fields
        if (!application || !application.rid) {
            return callback(M.error(M.error.DB_ORIENT_INVALID_RECORD, 'application', JSON.stringify(application)));
        }
...
function getFromHost (host, fields, callback) {
    ...
    var className = 'VDomain';
    var command = 'SELECT ' + M.orient.sqlSelectFields(fields, 'application') + ' FROM ' + className + ' WHERE name = "' + host + '"';
    ...
    M.orient.sqlCommand(command, function (err, data) {
        ...
        if (data.length > 1) {
            return callback(M.error(M.error.DB_ORIENT_NOT_UNIQUE, className, 'domain = "' + host + '"'));
        }
...
exports.translateAppId = translateAppId;
