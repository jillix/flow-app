...
function getRepo (data, callback) {
    ...
    request.get('https://api.bitbucket.org/1.0/repositories/' + data.user + '/' + data.repo, { auth: data.auth }, function(err, response, body) {
        ...
        try {
            ...
            callback(M.error(M.error.API_JSON_PARSE, e.message));
        }
    });
}
...
function getHeadCommit (data, callback) {
    ...
    request.get('https://api.bitbucket.org/1.0/repositories/' + data.user + '/' + data.repo + '/branches', { auth: data.auth }, function(err, response, body) {
