...
function getContent (data, callback) {
    ...
    getRepo(data, function (err, repo) {
        ...
        request.get('https://api.bitbucket.org/1.0/repositories/' + repo.owner + '/' + repo.name + '/raw/master/' + data.path, { oauth: oauth }, function(err, response, body) {
...
function getRepo (data, callback) {
    ...
    request.get(url, { oauth: oauth }, function(err, response, body) {
