...
function getUserRepos (user, data, callback) {
    ...
    if (data.auth) {
        bitbucket.authenticateOAuth(getOauth(data.secrets), link.session.auth.oauthAccessToken, link.session.auth.oauthAccessTokenSecret);
    }
