...
function authenticate (auth) {
    ...
    var BBApi = "Bitbucket API: ";
    
    if (!auth.type) { return console.log(BBApi + "Missing auth type. ('basic' or 'oauth')"); }
    if (["basic", "oauth"].indexOf(auth.type) === -1 ) { return console.log(BBApi + "Invalid auth type: " + auth.type); }

    if (auth.type === "oauth") {
        if (!auth.secrets) { return console.log(BBApi + "Missing secrets when authenticating."); }
        if (!auth.access_token) { return console.log(BBApi + "Missing auth.access_token."); }
        if (!auth.access_token_secret) { return console.log(BBApi + "Missing auth.access_token_secret."); }
    }
    else
    if (!auth.username || !auth.password) { return console.log(BBApi + "Missing 'username' or 'password' keys in the auth object."); }

    bb.authenticateOAuth(getOauth(auth.secrets), auth.access_token, auth.access_token_secret);
}
...
function getUserRepos (user, data, callback) {
    ...
    bb.getRepoApi().getAllFromUser(user, function (err, repositories) {
        ...
        for (var i in repositories) {
            if (data.type === 'all' ||
                    data.type === 'member' && repositories[i].owner !== user ||
                    data.type === 'owner' && repositories[i].owner === user) {
...
function getOauth(secrets) {
    ...
    if (!secrets) { secrets = require(M.config.APPLICATION_ROOT + "00000000000000000000000000000002/secrets.json").bitbucket; }
