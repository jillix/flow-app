...
const Store = require(resolve(__dirname + '/../node_modules/flow-api/lib/store'));
const libob = require('libobject');
const cache_options = {max: 500};
const RE_method_path = /^<([^\/]+)\/([^#]+)(?:#([^\?]+))?\?(.*)>$/;

// TODO create a dev and a producetion adapter
// create store instance from config file
const store = Store(require(resolve(__dirname + '/../config.json')));
...
module.exports = (application_dir) => {

    return {
        cache: LRU(cache_options),
        seq: (scope, sequence_id, role) => {
            return store.sequence(sequence_id, role, true);
        },
