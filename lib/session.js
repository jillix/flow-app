...
var sessionCache = engine.pojo('sessions');
var publicSid = engine.uid();
var sessionCookieKey = 'sid';

var roleKey = engine.session_role;
var userKey = engine.session_user;
var localeKey = engine.session_locale;
...
module.exports = function (connection, response) {
    ...
    if (connection.upgradeReq) {
        ...
        connection.on('message', function (message) {
            engine.link.message(message, connection, session);
        });
    ...
    engine.http.request(session, connection, response);
};
