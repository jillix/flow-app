...
exports.getConfig = function(link) {
    ...
    var getModuleConfig = function(miid) {
        ...
        M.module.getConfig(M.config.app.id, miid, link.session.rid, function(err, module) {
            ...
            if (module.config && module.config.html) {
                ...
                var pathLang = path + module.config.html.path + (link.session.loc === "*" ? "" : "." + link.session.loc) + ".html";
    ...
    // send cached config
    if (configCache[miid]) {
        return link.send(200, {type: "config", data: configCache[miid]});
    }
...
exports.getModule = function(link) {
    ...
    M.module.getFile(source, owner, name, link.session.rid, function(err, module) {
