var read = require(M.config.root + "/lib/util.js").read;
var stat = require("node-static").Server;
var client = new stat(M.config.root + "/lib/client");
var modules = new stat(M.config.root + "/apps/" + M.config.app + "/mono_modules");
...
exports.getConfig = function(link) {
    ...
    M.model.getAppId(link.host, function(err, appid, errMiid) {
 
        if (err || !appid) {
            return link.send(500, err || "Missing appid");
        }

        var getModuleConfig = function(miid) {
            
            M.model.getModuleConfig(appid, miid, link.session.uid, function(err, module) {

                // error checks
                if (err || !module) {
                    if (errMiid) {
                        link.send(200, { type: "miid", data: errMiid });
                    } else {
                        link.send(200, { type: "text", data: "No such module found: " + miid });
                    }
                    return;
                }
...
exports.getModule = function(link) {
    ...
    M.model.getModuleFile(source, owner, name, link.session.uid, function(err, module) {
