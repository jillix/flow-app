...
function moduleFiles () {
    ...
    if (cachedMiid) {
        return serveFile.call(self, cachedMiid.mono.name + path);
    }
    
    self.link.send(404, 'Miid not found');
}

// load module dependency (http)
function dependency () {
    var self = this;
    
    var miid = self.link.path[0];
    var source = self.link.path[1];
    var owner = self.link.path[2];
    var name = self.link.path[3];
    var version = self.link.path[4];
    var url = self.link.path.slice(1).join('/');
    var access = false;
    ...
    // chech if url is in the dependency list
    if (cachedMiid && cachedMiid.mono.client.scripts) {
        for (var i = 0, l = cachedMiid.mono.client.scripts.length; i < l; ++i) {
            if (cachedMiid.mono.client.scripts[i] === url) {
                access = true;
                break;
            }
        }
    ...
    // check if url is in the dependency list
    if (!access) {
        return self.link.send(404, "Dependency not found");
    }
    
    serveFile.call(self, url);
}
