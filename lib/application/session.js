...
var Session = {
    ...
    set: function( callback ){
        ...
        M.session.endSession( this.sid, callback );
    },

    endAll: function( ){
        M.session.endAllUserSessions( this.uid, callback );
    },
...
//
//    M.session.endSessions( expire(), function( err ){
...
exports.get = function(link, callback) {
    ...
    var publicSession = function(domain, callback) {
        M.model.getDomainPublicUser(domain, callback);
    };

    // try to find session
    if (sid) {
        M.session.getSession(sid, expire(), expire(expire_time), function(err, session) {
...
exports.start = function(uid, appid, locale, data, callback) {
    ...
    M.session.startSession(session, function(err) {
...
function update( sid, data, newId, callback ){
	...
	M.session.updateSession( sid, q_update, function( err ){
