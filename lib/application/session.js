...
function setup (callback) {
    ...
    M.store(M.config.store.systemStoreName, function (err, store) {
        ...
        sessionModel = store.collection(sessionCollection);
        
        // append session factory to api
        M.session = factory;
    ...
    return function (connection, response) {
        ...
        if (con.headers.cookie) {
            ...
            if (parsedCookie && parsedCookie[sessionCookieKey]) {
                ...
                // TODO update expire
                return sessionModel.findOne({sid: sessionId/*TODO expire, exp: {$lt: now}*/}, function (err, item) {
