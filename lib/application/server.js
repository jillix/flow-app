...
function getCachedInstance (session, instance, event) {
    ...
    if (instance && (instance.mono.name === 'M' || instance.mono.roles[session[M.config.session.role]]) && instance.mono.access[event]) {
        return instance;
    }
    return null;
}
...
// TODO buffer messages while fetching the session
