...
M.orient.connect(function(err, db) {
    ...
    M.app.getFromId(M.config.app, {publicUser: 1, session: 1}, function (err, application) {
        ...
        if (isNaN(publicUser)) {
            console.error("Could not determine the public user for application: " + M.config.app);
            process.exit(4);
        }
