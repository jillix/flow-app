...
// TODO check caching
function connect (connection, callback, log) {
    ...
    if (cbBuffer[config.db] instanceof Array) {
        if (log) {
            console.log('- store callback in buffer');
        }
    ...
    if (log) {
        console.log('- store new client');
    }
    
    // TODO mongo client should only open once
    // to get a db then is sync
    // maybe make another function for retrieving a db
...
function parseConnection (connection) {
    ...
    // TODO this is is a hack!
    if (!connection.match(CONNECTION_STRING_RE)) {
