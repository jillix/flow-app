    ...
    fs.readFile(file.read, function (err, data) {
        ...
        switch (file.type) {
            ...
            case 'application/javascript':
                ...
                if (file.wrap && file.clean !== 'engine.js') {
...
exports.prepareComponents = function (loadContext, callback) {
    ...
    var handler = function (err, file) {
        ...
        file.ext = (file.path[0] !== '/' ? modulePath : '') + (file.ext || file.path);
    ...
    for (var type in loadContext.components) {
        ...
        for (var i = 0, file; i < files.length; ++i) {
            ...
            // append base path
            file.base = (file.path[0] !== '/' ? loadContext._base : null) || (type === 'markup' ? engine.paths.app_markup : engine.paths.app_public);

            // wrap javascript files
            if (file.path[0] !== '/' && type === 'scripts') {
                file.wrap = modulePath + file.path;
            }
