...
// get json package and composition
// get file for http response
...
exports.prepare = function (components, callback) {
    ...
    if (components.module instanceof Array) {
        files.module = components.module;
    }
    ...
    if (!files.module && !files.styles && !files.markup) {
        return callback(null, components);
    }
    
    // set apps public folder as default base path
    var base = components._base || engine.paths.app_public;
    ...
    var handler = function (err, file, context) {
        ...
        // TODO merget external file path to components (_base + fingerprint)
        
        if (--countDown === 0) {
            console.log('PREPARE DONE', files);
            callback(null, components);
        }
    };
...
exports.get = function (file, callback, context) {
    ...
    fs.readFile(file.read, function (err, data) {
        ...
        if (suffix.test(file.path)) {
            ...
            file.ext = (file.module || '') + file.ext.join('.');
        } else {
            file.ext = (file.module || '') + file.path + '.@' + file.fp;
        }
