...
var Cache = {
    ...
    set: function (path, callback, allowedFiles, prependPath) {
        ...
        if (cleanPath.indexOf('../') !== -1) {
            ...
            console.error('lib/file.js#166', err.toString());
            return callback(err);
        }
...
function createCacheObject (cleanPath, readPath, data, mimeType, fingerprint, callback) {
    ...
    fs.stat(readPath, function (err, stats) {
        ...
        // handle files without fingerprint
        if (!fingerprint) {
            data.http.Etag = stats.mtime.getTime().toString(36).slice(-7);
            data.http['Last-Modified'] = stats.mtime;
        }
