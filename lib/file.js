...
function getFile (parentDir, path, callback, options) {
    ...
    var file = fileCache.get(cleanPath);
...
function readFile (file, parentDir, path, cleanPath, options, hasFingerprint, callback) {
    ...
    fs.readFile(path, function (err, data) {
        ...
        if (err) {
            console.error('lib/file.js#183', err.toString());
            return callback(err);
        }
        ...
        switch (mimeType) {
            ...
            case 'application/javascript':
                ...
                if (cleanPath !== 'engine.js') {
                    data = "E('" + (options.prependPath || '') + cleanPath + "',function(require,module,exports){\n" + data.toString() + "\nreturn module});";
                }
 ...
 */
exports.addFingerprints = function (module, files, callback, query) {
    ...
    var handler = function (index) {
        ...
        switch (files[index][0]) {
            case '.':
                ...
                file = files[index].substr(2);
                options.prependPath = module + '/';
                break;
            case '/':
                cwd = engine.paths.app_public;
                file = files[index].substr(1);
                break;
            default:
                cwd = engine.root + 'lib/client/';
                file = files[index];
                options.prependPath = 'E/';
        }

        // get the modification time
        // TODO create hash from file content (MD5)
        getFile(cwd, file,
            function (err, data) {
                ...
                stats = '@' + data.http.Etag.substr(0, 7);
