...
exports.prepare = function (components, callback) {
    ...
    if (!files.scripts && !files.styles && !files.markup) {
        return utils.nextTick(callback, null, components);
    }
    ...
    var handler = function (err, file, context) {
        ...
        if (!file.worker && engine.production && context.type === 'scripts' && file.data) {
            combinedFile.files.push(file);
            files[context.type][context.index] = null;
        ...
        if (--countDown === 0) {
            ...
            if (engine.production && files.scripts && combinedFile) {
                ...
                delete combinedFile.files;

                cbBuffer[combinedFile.read] = cbBuffer[combinedFile.read] || [];
                cbBuffer[combinedFile.read].push(function (err, file) {
                    ...
                    var scripts = [];
                    for (var i = 0, l = files.scripts.length; i < l; i++) {
                        if (files.scripts[i]) {
                            scripts.push(files.scripts[i]);
                        }
                    }
                    scripts.push(file.ext);
                ...
                createFileDescriptor(combinedFile, combinedFile.data);
    ...
    for (type in files) {
        ...
        for (i = 0; i < l; ++i) {
            ...
            if (files[type][i].indexOf('//') > -1) {
                utils.nextTick(handler, null, {ext: files[type][i]}, context);
                continue;
            }
...
exports.json = function (path, callback, type, key) {
    ...
    // TODO MODULE AND INSTANCE CB BUFFERING
...
exports.get = function (file, callback, context) {
    ...
    if (!file.path || !file.base) {
        return utils.nextTick(callback, new Error('Missing file path or file base.'), undefined, context);
    }
    ...
    if (cachedFile) {
        ...
        return utils.nextTick(callback, null, cachedFile, context);
    }
    
    cbBuffer[file.read] = cbBuffer[file.read] || [];
    
    // buffer callback if file is loading
    if (cachedFile === null) {
        cbBuffer[file.read].push(callback);
        return;
    }
    ...
    fs.readFile(file.read, function (err, data) {
        ...
        if (err) {
            return callbacks(file.read, engine.log('E', err), undefined, context);
        }
        ...
        fs.stat(file.read, function (err, stats) {
            ...
            createFileDescriptor(file, data, context);
        });
    });
};
 ...
 */
function createFileDescriptor (file, data, context) {
    ...
    if (file.noCompression) {
        return utils.nextTick(createCacheObject, file, context);
    }
    ...
    zlib.gzip(file.data, {level: zlib.Z_DEFAULT_COMPRESSION}, function (err, data) {
        ...
        createCacheObject(file, context);
    });
}
 ...
 * @private
 */
function createCacheObject (file, context) {
    ...
    // return data
    callbacks(file.read, null, file, context);
}

function callbacks (path, err, data, context) {
    if (cbBuffer[path]) {
        for (var i = 0; i < cbBuffer[path].length; i++) {
            cbBuffer[path][i](err, data, context);
        }
