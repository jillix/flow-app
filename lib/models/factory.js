    ...
    self.queries[config.name] = function (data, callback) {
        ...
        if (config.add && data) {
            ...
            for (var key in config.add) {
                ...
                if ((value = object.path(config.add[key], data))) {
                    request[key] = value;
                }
            }
        }

        // merge the static data to the request
        if (config.data) {
            for (var key in config.data) {
                request[key] = config.data[key];
            }
        }
...
function messageHandler (err, message, callback) {
    var self = this;
    ...
    model.queries[message.q](message.p, callback);
}
