...
function getFromHost (host, callback) {
    ...
    model.factory(projectsModel, function (err, projects) {
        ...
        // TODO implement wildcard domains
        projects.request({m: 'findOne', q: {domains: host}}, function (err, project) {

            if (err || !project || !project.process) {
                return callback(err || M.error(M.error.API_APP_NOT_FOUND, host));
            }

            project.process.id = project._id.toString();
...
function startApp (host, callback) {
    ...
    getFromHost(host, function(err, dbApplication) {
        ...
        if (!application.lib_tmp) {
            M.config.paths.PROJECT_SERVER = M.config.paths.MONO_ROOT + 'lib/project/server.js';
        }
        ...
        getPid(application.id, function (err, pid) {
            ...
            getFreePort(function (freePort) {
                ...
                // save application port in app config
                application.port = freePort;

                // spawn process with uid/gid and set the working directory to mono/users/uid/projects/appid/
                var spawnOptions = {
                    cwd: M.config.paths.USERS_ROOT + application.owner + '/projects/' + application.id + '/',
                ...
                // spawn process
                var app = spawn('node', [M.config.paths.PROJECT_SERVER], spawnOptions);

                // save process id in app cache
                application.pid = app.pid;

                // create writable log stream
                var appLogStream = fs.createWriteStream(M.config.paths.USERS_ROOT + application.owner + '/logs/' + application.name + '.txt', {flags: 'a'});
                ...
                app.on('exit', function (code) {
                    ...
                    if (!app._started) {
                        callback(new Error('app terminated'));
                    }
                });
            });
        });
