function loadTab( target, inst_id, loader ){
	...
	N.comp( target, inst_id, function( err, inst ){
		...
		if( !err ) {
			...
			if( inst && inst.obs ) inst.obs.l( "load", function(){
...
this.init = function( config ) {
    ...
    view.obs.l( "itemHandler", function( item, data, tab ) {
        ...
        //add logout event
        if( data.l ) item.bind( "click", function(){
        	
        	N.link( "/logout", function( err ){
        		
        		if( !err ) N.comp( "body", null, 1 );
        	});
        });
        
        //add right class to item
        if( data.r ) item.addAttr({ "class": "right" });
        
        //attach event to tab button
        if( view.inst[ data.h ] ) item.bind( config.event, instance[ data.h ] );
    });
};